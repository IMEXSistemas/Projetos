...
namespace BmsSoftware.Modulos.Operacional
{
    public partial class FrmNivel : Form
    {
        ...
        NIVELUSUARIOProvider NivelP = new NIVELUSUARIOProvider();
        ...
        public int _IDNIVELUSUARIO = -1;
        public NIVELUSUARIOEntity Entity
        {
            get
            {
                string NOME = txtNome.Text;
                string OBSERVACAO = txtObservacao.Text;
                return new NIVELUSUARIOEntity(_IDNIVELUSUARIO, NOME, OBSERVACAO);
            }
            set
            {
                ...
                if (value != null)
                {
                    _IDNIVELUSUARIO = value.IDNIVELUSUARIO;
        ...
        private void Grava()
        {
            try
            {
                if (Validacoes())
                {
                    _IDNIVELUSUARIO =  NivelP.Save(Entity);
                    GetAllMarca();
        ...
        {
            NIVELUSUARIOColl = NivelP.ReadCollectionByParameter(null, "NOME");
            DataGriewDados.AutoGenerateColumns = false;
            DataGriewDados.DataSource = NIVELUSUARIOColl;

            lblTotalPesquisa.Text = NIVELUSUARIOColl.Count.ToString();
        }
        ...
        private void FrmTipoRegiao_Load(object sender, EventArgs e)
        {
            ...
            if(_IDNIVELUSUARIO !=-1)
            {
                Entity = NivelP.Read(_IDNIVELUSUARIO);
            }
            ...
            else
            {
                ...
                if (dr == DialogResult.Yes)
                {
                    try
                    {
                        ...
                        DeleteControleAcesso(_IDNIVELUSUARIO);

                        NivelP.Delete(_IDNIVELUSUARIO);
        ...
        {
            if (NIVELUSUARIOColl.Count > 0)
            {
                int rowindex = e.RowIndex;
                if (rowindex != -1)
                {
                    int CodigoSelect = Convert.ToInt32(NIVELUSUARIOColl[rowindex].IDNIVELUSUARIO);

                    Entity = NivelP.Read(CodigoSelect);
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            try
            {
                ...
                int NumerorRegistros = NIVELUSUARIOColl.Count;

                while (IndexRegistro < NIVELUSUARIOColl.Count)
                {
                    config.PosicaoDaLinha = config.MargemSuperior + (config.LinhaAtual * config.FonteNormal.GetHeight(e.Graphics));
                    e.Graphics.DrawString(NIVELUSUARIOColl[IndexRegistro].IDNIVELUSUARIO.ToString(), config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha);
                    e.Graphics.DrawString(Util.LimiterText(NIVELUSUARIOColl[IndexRegistro].NOME, 40), config.FonteConteudo, Brushes.Black, config.MargemEsquerda + 50, config.PosicaoDaLinha);
                    e.Graphics.DrawString(Util.LimiterText(NIVELUSUARIOColl[IndexRegistro].OBSERVACAO, 100), config.FonteConteudo, Brushes.Black, config.MargemEsquerda + 250, config.PosicaoDaLinha);
                ...
                if (IndexRegistro < NIVELUSUARIOColl.Count)
                    e.HasMorePages = true;
                else
                {
                    e.Graphics.DrawString("", config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha + 50);
                    e.Graphics.DrawString("Total da pesquisa: " + NIVELUSUARIOColl.Count, config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha + 50);
        ...
        private void DataGriewDados_KeyDown(object sender, KeyEventArgs e)
        {
            if (NIVELUSUARIOColl.Count > 0)
            {
                ...
                int CodigoSelect = Convert.ToInt32(NIVELUSUARIOColl[indice].IDNIVELUSUARIO);
