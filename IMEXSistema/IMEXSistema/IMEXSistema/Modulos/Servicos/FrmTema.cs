...
namespace BmsSoftware.Modulos.Servicos
{
    public partial class FrmTema : Form
    {
        ...
        TEMAProvider TEMAP = new TEMAProvider();
        ...
        int _IDTEMA = -1;
        public TEMAEntity Entity
        {
            get
            {
                string NOME = txtNome.Text;
                string OBSERVACAO = txtObservacao.Text;
                return new TEMAEntity(_IDTEMA, NOME, OBSERVACAO);
            }
            set
            {
        ...
        {
            TEMAColl = TEMAP.ReadCollectionByParameter(null, "NOME");
            DataGriewDados.AutoGenerateColumns = false;
            DataGriewDados.DataSource = TEMAColl;

            lblTotalPesquisa.Text = TEMAColl.Count.ToString();
        }
        ...
        private void DataGriewDados_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if (TEMAColl.Count > 0)
            {
                ...
                if (rowindex != -1)
                {
                    int CodigoSelect = Convert.ToInt32(TEMAColl[rowindex].IDTEMA);

                    Entity = TEMAP.Read(CodigoSelect);
        ...
        {
            RelatorioTitulo = InputBox("Título do Relatório", ConfigSistema1.Default.NomeEmpresa, "Lista de Temas");
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            try
            {
                ...
                int NumerorRegistros = TEMAColl.Count;

                while (IndexRegistro < TEMAColl.Count)
                {
                    config.PosicaoDaLinha = config.MargemSuperior + (config.LinhaAtual * config.FonteNormal.GetHeight(e.Graphics));
                    e.Graphics.DrawString(TEMAColl[IndexRegistro].IDTEMA.ToString(), config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha);
                    e.Graphics.DrawString(Util.LimiterText(TEMAColl[IndexRegistro].NOME, 40), config.FonteConteudo, Brushes.Black, config.MargemEsquerda + 50, config.PosicaoDaLinha);
                ...
                if (IndexRegistro < TEMAColl.Count)
                    e.HasMorePages = true;
                else
                {
                    e.Graphics.DrawString("", config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha + 50);
                    e.Graphics.DrawString("Total da pesquisa: " + TEMAColl.Count, config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha + 50);
        ...
        private void DataGriewDados_KeyDown(object sender, KeyEventArgs e)
        {
            if (TEMAColl.Count > 0)
            {
                ...
                int CodigoSelect = Convert.ToInt32(TEMAColl[indice].IDTEMA);
