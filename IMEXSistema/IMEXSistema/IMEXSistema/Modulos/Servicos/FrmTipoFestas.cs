...
{
    public partial class FrmTipoFestas : Form
    {
        ...
        int _IDFESTAS = -1;
        public FESTASEntity Entity
        {
            get
            {
                string NOME = txtNome.Text;
                string OBSERVACAO = txtObservacao.Text;
                return new FESTASEntity(_IDFESTAS, NOME, OBSERVACAO);
            }
            set
            {
        ...
        private void GetAllFestas()
        {
            FESTASColl = FESTASP.ReadCollectionByParameter(null, "NOME");
            DataGriewDados.AutoGenerateColumns = false;
            DataGriewDados.DataSource = FESTASColl;

            lblTotalPesquisa.Text = FESTASColl.Count.ToString();
        }
        ...
        {
            if (FESTASColl.Count > 0)
            {
                int rowindex = e.RowIndex;
                if (rowindex != -1)
                {
                    int CodigoSelect = Convert.ToInt32(FESTASColl[rowindex].IDFESTAS);

                    Entity = FESTASP.Read(CodigoSelect);
        ...
        {
            RelatorioTitulo = InputBox("Título do Relatório", ConfigSistema1.Default.NomeEmpresa, "Lista de Festas");
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            try
            {
                ...
                int NumerorRegistros = FESTASColl.Count;

                while (IndexRegistro < FESTASColl.Count)
                {
                    config.PosicaoDaLinha = config.MargemSuperior + (config.LinhaAtual * config.FonteNormal.GetHeight(e.Graphics));
                    e.Graphics.DrawString(FESTASColl[IndexRegistro].IDFESTAS.ToString(), config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha);
                    e.Graphics.DrawString(Util.LimiterText(FESTASColl[IndexRegistro].NOME, 40), config.FonteConteudo, Brushes.Black, config.MargemEsquerda + 50, config.PosicaoDaLinha);
                ...
                if (IndexRegistro < FESTASColl.Count)
                    e.HasMorePages = true;
                else
                {
                    e.Graphics.DrawString("", config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha + 50);
                    e.Graphics.DrawString("Total da pesquisa: " + FESTASColl.Count, config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha + 50);
        ...
        private void DataGriewDados_KeyDown(object sender, KeyEventArgs e)
        {
            if (FESTASColl.Count > 0)
            {
                ...
                int CodigoSelect = Convert.ToInt32(FESTASColl[indice].IDFESTAS);
