...
namespace BmsSoftware.Modulos.Financeiro
{
    public partial class FrmBanco : Form
    {
        ...
        public BANCOEntity Entity
        {
            get
            {
                string NOMEBANCO =txtNomeBanco.Text;
                string NUMEROBANCO = txtNumeroBanco.Text;
                return new BANCOEntity(_IDBANCO, NOMEBANCO, NUMEROBANCO);
            }
            set
            {
                ...
                if (value != null)
                {
                    _IDBANCO = value.IDBANCO;
                    txtNumeroBanco.Text = value.NUMEROBANCO;
                    txtNomeBanco.Text = value.NOMEBANCO;
                    errorProvider1.Clear();
                }
                else
                {
                    _IDBANCO = -1;
                    txtNumeroBanco.Text = string.Empty;
        ...
        private Boolean Validacoes()
        {
            ...
            errorProvider1.Clear();
            if (txtNumeroBanco.Text.Trim().Length == 0)
            {
                errorProvider1.SetError(txtNumeroBanco, ConfigMessage.Default.CampoObrigatorio);
                Util.ExibirMSg(ConfigMessage.Default.CampoObrigatorio2, "Red");
                result = false;
            }
            else
                errorProvider1.SetError(txtNumeroBanco, "");
        ...
        {
            if (BANCOColl.Count > 0)
            {
                int rowindex = e.RowIndex;
                if (rowindex != -1)
                {
                    int CodigoSelect = Convert.ToInt32(BANCOColl[rowindex].IDBANCO);

                    Entity = BANCOP.Read(CodigoSelect);
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            try
            {
                ...
                e.Graphics.DrawString("Nome", config.FonteNegrito, Brushes.Black, config.MargemEsquerda + 250, 170);
                ...
                int NumerorRegistros = BANCOColl.Count;

                while (IndexRegistro < BANCOColl.Count)
                {
                    config.PosicaoDaLinha = config.MargemSuperior + (config.LinhaAtual * config.FonteNormal.GetHeight(e.Graphics));
                    e.Graphics.DrawString(BANCOColl[IndexRegistro].IDBANCO.ToString(), config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha);
                    e.Graphics.DrawString(Util.LimiterText(BANCOColl[IndexRegistro].NUMEROBANCO, 40), config.FonteConteudo, Brushes.Black, config.MargemEsquerda + 50, config.PosicaoDaLinha);
                    e.Graphics.DrawString(Util.LimiterText(BANCOColl[IndexRegistro].NOMEBANCO, 100), config.FonteConteudo, Brushes.Black, config.MargemEsquerda + 250, config.PosicaoDaLinha);
                ...
                if (IndexRegistro < BANCOColl.Count)
                    e.HasMorePages = true;
                else
                {
                    e.Graphics.DrawString("", config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha + 50);
                    e.Graphics.DrawString("Total da pesquisa: " + BANCOColl.Count, config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha + 50);
        ...
        private void DataGriewDados_KeyDown(object sender, KeyEventArgs e)
        {
            if (BANCOColl.Count > 0)
            {
                ...
                int CodigoSelect = Convert.ToInt32(BANCOColl[indice].IDBANCO);
