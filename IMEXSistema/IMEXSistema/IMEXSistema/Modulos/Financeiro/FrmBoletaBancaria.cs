...
namespace BmsSoftware.Modulos.Financeiro
{
    public partial class FrmBoletaBancaria : Form
    {
        ...
        private void ImprimiBoletaBrasil()
        {
            ...
            if (txtCodCedente.Text != string.Empty)
                c.Codigo= Convert.ToInt32(Entity.CODCEDENTE);

            string DataVencimentoTeste = DateTime.Now.AddDays(1).ToString();
            Boleto b = new Boleto(Convert.ToDateTime(DataVencimentoTeste), Convert.ToDouble(ValorTeste),
        ...
        private void txtDigitoCodCedente_DoubleClick(object sender, EventArgs e)
        {
            if(txtCodCedente.Text.Trim() != string.Empty)
                txtDigitoCodCedente.Text = Util.DVCodCedenteCAIXA(txtCodCedente.Text).ToString();
        }

        private void txtDigitoCodCedente_Enter(object sender, EventArgs e)
        {
            lblObsField.Text = "Duplo Clique para Gerar o DV do Cedente!";
        }

        private void txtDigitoCodCedente_Leave(object sender, EventArgs e)
        {
            lblObsField.Text = "";
        }
    }
