...
{
    public partial class FrmTamanho : Form
    {
        ...
        public int _IDTAMANHO = -1;
        public TAMANHOEntity Entity
        {
            get
            {
                string NOME = txtNome.Text;
                string OBSERVACAO = txtObservacao.Text;
                return new TAMANHOEntity(_IDTAMANHO, NOME, OBSERVACAO);
            }
            set
            {
        ...
        private void Grava()
        {
            try
            {
                if (Validacoes())
                {
                    _IDTAMANHO = TAMANHOP.Save(Entity);
                    GetAllRegistro();
        ...
        private void GetAllRegistro()
        {
            TAMANHOColl = TAMANHOP.ReadCollectionByParameter(null, "NOME");
            DataGriewDados.AutoGenerateColumns = false;
            DataGriewDados.DataSource = TAMANHOColl;

            lblTotalPesquisa.Text = TAMANHOColl.Count.ToString();
        }
        ...
        private void FrmTipoRegiao_Load(object sender, EventArgs e)
        {
            ...
            if (_IDTAMANHO != -1)
                Entity = TAMANHOP.Read(_IDTAMANHO);
        ...
        {
            if (TAMANHOColl.Count > 0)
            {
                int rowindex = e.RowIndex;
                if (rowindex != -1)
                {
                    int CodigoSelect = Convert.ToInt32(TAMANHOColl[rowindex].IDTAMANHO);

                    Entity = TAMANHOP.Read(CodigoSelect);
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            try
            {
                ...
                int NumerorRegistros = TAMANHOColl.Count;

                while (IndexRegistro < TAMANHOColl.Count)
                {
                    config.PosicaoDaLinha = config.MargemSuperior + (config.LinhaAtual * config.FonteNormal.GetHeight(e.Graphics));
                    e.Graphics.DrawString(TAMANHOColl[IndexRegistro].IDTAMANHO.ToString(), config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha);
                    e.Graphics.DrawString(Util.LimiterText(TAMANHOColl[IndexRegistro].NOME, 40), config.FonteConteudo, Brushes.Black, config.MargemEsquerda + 50, config.PosicaoDaLinha);
                    e.Graphics.DrawString(Util.LimiterText(TAMANHOColl[IndexRegistro].OBSERVACAO, 100), config.FonteConteudo, Brushes.Black, config.MargemEsquerda + 250, config.PosicaoDaLinha);
                ...
                if (IndexRegistro < TAMANHOColl.Count)
                    e.HasMorePages = true;
                else
                {
                    e.Graphics.DrawString("", config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha + 50);
                    e.Graphics.DrawString("Total da pesquisa: " + TAMANHOColl.Count, config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha + 50);
        ...
        private void DataGriewDados_KeyDown(object sender, KeyEventArgs e)
        {
            if (TAMANHOColl.Count > 0)
            {
                ...
                int CodigoSelect = Convert.ToInt32(TAMANHOColl[indice].IDTAMANHO);
