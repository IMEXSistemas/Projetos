...
{
    public partial class FrmVendaporCidade : Form
    {
        LIS_PEDIDO3Collection LIS_PEDIDO3Coll = new LIS_PEDIDO3Collection();
        LIS_PEDIDO3Provider LIS_PEDIDOP3 = new LIS_PEDIDO3Provider();
        ...
        {
            string RelatorioTitulo = InputBox("Título do Relatório", ConfigSistema1.Default.NomeEmpresa, "Lista de Vendas por Cidade");
        ...
        private void btnPesquisa_Click(object sender, EventArgs e)
        {
            if (Validacoes())
            {
                try
                {
                    ...
                    if (Convert.ToInt32(cbFuncionario.SelectedValue) > 0)
                        LIS_PEDIDO3Coll = LIS_PEDIDOP3.ReadCollectionByParameter(RowRelatorio, "IDTRANSPORTES, IDPEDIDO");
                    else
                        LIS_PEDIDO3Coll = LIS_PEDIDOP3.ReadCollectionByParameter(RowRelatorio);
