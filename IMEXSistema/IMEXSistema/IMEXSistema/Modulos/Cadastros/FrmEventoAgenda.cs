...
{
    public partial class FrmEventoAgenda : Form
    {
        Utility Util = new Utility();
        EVENTOAGENDAProvider EVENTOAGENDAP = new EVENTOAGENDAProvider();
        EVENTOAGENDACollection EVENTOAGENDAColl = new EVENTOAGENDACollection();  

        public FrmEventoAgenda()
        {
            InitializeComponent();
        ...
        int _IDEVENTOAGENDA = -1;
        public EVENTOAGENDAEntity Entity
        {
            get
            {
                string NOME = txtNome.Text;
                string OBSERVACAO = txtObservacao.Text;
                return new EVENTOAGENDAEntity(_IDEVENTOAGENDA, NOME, OBSERVACAO);
            }
            set
            {
                ...
                if (value != null)
                {
                    _IDEVENTOAGENDA = value.IDEVENTOAGENDA;
        ...
        private void Grava()
        {
            try
            {
                if (Validacoes())
                {
                    _IDEVENTOAGENDA = EVENTOAGENDAP.Save(Entity);
                    GetAllEventoAgenda();
        ...
        private void GetAllEventoAgenda()
        {
            EVENTOAGENDAColl = EVENTOAGENDAP.ReadCollectionByParameter(null, "NOME");
            DataGriewDados.AutoGenerateColumns = false;
            DataGriewDados.DataSource = EVENTOAGENDAColl;

            lblTotalPesquisa.Text = EVENTOAGENDAColl.Count.ToString();
        }
        ...
        {
            Entity = null;
            tabControlEventoAgenda.SelectTab(0);
            txtNome.Focus();
        }
        ...
        private void Delete()
        {
            if (_IDEVENTOAGENDA == -1)
            {
                MessageBox.Show(ConfigMessage.Default.MsgSelecRegistro);
                tabControlEventoAgenda.SelectTab(1);
            }
            else
            {
                ...
                if (dr == DialogResult.Yes)
                {
                    try
                    {
                        EVENTOAGENDAP.Delete(_IDEVENTOAGENDA);
        ...
        {
            if (EVENTOAGENDAColl.Count > 0)
            {
                int rowindex = e.RowIndex;
                if (rowindex != -1)
                {
                    int CodigoSelect = Convert.ToInt32(EVENTOAGENDAColl[rowindex].IDEVENTOAGENDA);

                    Entity = EVENTOAGENDAP.Read(CodigoSelect);
        ...
        {
            RelatorioTitulo = InputBox("Título do Relatório", ConfigSistema1.Default.NomeEmpresa, "Lista de Eventos da Agenda");
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            try
            {
                ...
                int NumerorRegistros = EVENTOAGENDAColl.Count;

                while (IndexRegistro < EVENTOAGENDAColl.Count)
                {
                    config.PosicaoDaLinha = config.MargemSuperior + (config.LinhaAtual * config.FonteNormal.GetHeight(e.Graphics));
                    e.Graphics.DrawString(EVENTOAGENDAColl[IndexRegistro].IDEVENTOAGENDA.ToString(), config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha);
                    e.Graphics.DrawString(Util.LimiterText(EVENTOAGENDAColl[IndexRegistro].NOME, 40), config.FonteConteudo, Brushes.Black, config.MargemEsquerda + 50, config.PosicaoDaLinha);
                    e.Graphics.DrawString(Util.LimiterText(EVENTOAGENDAColl[IndexRegistro].OBSERVACAO, 100), config.FonteConteudo, Brushes.Black, config.MargemEsquerda + 250, config.PosicaoDaLinha);
                ...
                if (IndexRegistro < EVENTOAGENDAColl.Count)
                    e.HasMorePages = true;
                else
                {
                    e.Graphics.DrawString("", config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha + 50);
                    e.Graphics.DrawString("Total da pesquisa: " + EVENTOAGENDAColl.Count, config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha + 50);
        ...
        private void FrmEventoAgenda_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void DataGriewDados_KeyDown(object sender, KeyEventArgs e)
        {
            if (EVENTOAGENDAColl.Count > 0)
            {
                ...
                int CodigoSelect = Convert.ToInt32(EVENTOAGENDAColl[indice].IDEVENTOAGENDA);
                ...
                else if (e.KeyCode == Keys.Delete)
                {
                    ...
                    if (dr == DialogResult.Yes)
                    {
                        try
                        {
                            EVENTOAGENDAP.Delete(CodigoSelect);
                            Util.ExibirMSg(ConfigMessage.Default.MsgDelete2, "Blue");
                            GetAllEventoAgenda();
                        }
                        catch (Exception)
