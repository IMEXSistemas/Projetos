...
namespace BmsSoftware.Modulos.Cadastros
{
    public partial class FrmParentesco : Form
    {
        ...
        PARENTESCOProvider PARENTESCOP = new PARENTESCOProvider();
        ...
        int _IDPARENTESCO = -1;
        public PARENTESCOEntity Entity
        {
            get
            {
                string DESCRICAO = txtDescricao.Text;
                return new PARENTESCOEntity(_IDPARENTESCO, DESCRICAO);
            }
            set
            {
                ...
                if (value != null)
                {
                    _IDPARENTESCO = value.IDPARENTESCO;
        ...
        private void Grava()
        {
            try
            {
                if (Validacoes())
                {
                   _IDPARENTESCO = PARENTESCOP.Save(Entity);
                   GetAllParentesco();
        ...
        private void GetAllParentesco()
        {
           PARENTESCOColl = PARENTESCOP.ReadCollectionByParameter(null, "DESCRICAO");
            DataGriewDados.AutoGenerateColumns = false;
            DataGriewDados.DataSource = PARENTESCOColl ;

            lblTotalPesquisa.Text = PARENTESCOColl .Count.ToString();
        }
        ...
        private void Delete()
        {
            ...
            else
            {
                ...
                if (dr == DialogResult.Yes)
                {
                    try
                    {
                       PARENTESCOP.Delete(_IDPARENTESCO);
        ...
        {
            if (PARENTESCOColl .Count > 0)
            {
                int rowindex = e.RowIndex;
                if (rowindex != -1)
                {
                    int CodigoSelect = Convert.ToInt32(PARENTESCOColl [rowindex].IDPARENTESCO);

                    Entity = PARENTESCOP.Read(CodigoSelect);
        ...
        {
            RelatorioTitulo = InputBox("Título do Relatório", ConfigSistema1.Default.NomeEmpresa, "Lista de Parentesco");
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            try
            {
                ...
                int NumerorRegistros = PARENTESCOColl .Count;

                while (IndexRegistro < PARENTESCOColl .Count)
                {
                    config.PosicaoDaLinha = config.MargemSuperior + (config.LinhaAtual * config.FonteNormal.GetHeight(e.Graphics));
                    e.Graphics.DrawString(PARENTESCOColl [IndexRegistro].IDPARENTESCO.ToString(), config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha);
                    e.Graphics.DrawString(Util.LimiterText(PARENTESCOColl [IndexRegistro].DESCRICAO, 40), config.FonteConteudo, Brushes.Black, config.MargemEsquerda + 50, config.PosicaoDaLinha);
                ...
                if (IndexRegistro < PARENTESCOColl .Count)
                    e.HasMorePages = true;
                else
                {
                    e.Graphics.DrawString("", config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha + 50);
                    e.Graphics.DrawString("Total da pesquisa: " + PARENTESCOColl .Count, config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha + 50);
        ...
        private void DataGriewDados_KeyDown(object sender, KeyEventArgs e)
        {
            if (PARENTESCOColl .Count > 0)
            {
                ...
                int CodigoSelect = Convert.ToInt32(PARENTESCOColl [indice].IDPARENTESCO);
                ...
                else if (e.KeyCode == Keys.Delete)
                {
                    ...
                    if (dr == DialogResult.Yes)
                    {
                        try
                        {
                            PARENTESCOP.Delete(CodigoSelect);
                            Util.ExibirMSg(ConfigMessage.Default.MsgDelete2, "Blue");
                            GetAllParentesco();
                        }
                        catch (Exception)
