...
{
    public partial class FrmGrupoAgenda : Form
    {
        Utility Util = new Utility();
        GRUPOAGENDAProvider GRUPOAGENDAPP = new GRUPOAGENDAProvider();
        GRUPOAGENDACollection GRUPOAGENDAColl = new GRUPOAGENDACollection();

        public FrmGrupoAgenda()
        {
            InitializeComponent();
        ...
        int _IDGRUPOAGENDA = -1;
        public GRUPOAGENDAEntity Entity
        {
            get
            {
                string NOMEGRUPOAGENDA = txtNome.Text;
                string OBSERVACAO = txtObservacao.Text;
                return new GRUPOAGENDAEntity(_IDGRUPOAGENDA, NOMEGRUPOAGENDA, OBSERVACAO);
            }
            set
            {
                ...
                if (value != null)
                {
                    _IDGRUPOAGENDA = value.IDGRUPOAGENDA;
                    txtNome.Text = value.NOMEGRUPOAGENDA;
        ...
        private void Grava()
        {
            try
            {
                if (Validacoes())
                {
                    _IDGRUPOAGENDA = GRUPOAGENDAPP.Save(Entity);
                    GetAllGrupoAgenda();
        ...
        private void GetAllGrupoAgenda()
        {
            GRUPOAGENDAColl = GRUPOAGENDAPP.ReadCollectionByParameter(null, "NOMEGRUPOAGENDA");
            DataGriewDados.AutoGenerateColumns = false;
            DataGriewDados.DataSource = GRUPOAGENDAColl;

            lblTotalPesquisa.Text = GRUPOAGENDAColl.Count.ToString();
        }
        ...
        {
            Entity = null;
            tabControlGrupoAgenda.SelectTab(0);
            txtNome.Focus();
        }
        ...
        private void Delete()
        {
            if (_IDGRUPOAGENDA == -1)
            {
                ...
                tabControlGrupoAgenda.SelectTab(1);
            }
            else
            {
                ...
                if (dr == DialogResult.Yes)
                {
                    try
                    {
                        GRUPOAGENDAPP.Delete(_IDGRUPOAGENDA);
        ...
        {
            if (GRUPOAGENDAColl.Count > 0)
            {
                int rowindex = e.RowIndex;
                if (rowindex != -1)
                {
                    int CodigoSelect = Convert.ToInt32(GRUPOAGENDAColl[rowindex].IDGRUPOAGENDA);

                    Entity = GRUPOAGENDAPP.Read(CodigoSelect);
        ...
        {
            RelatorioTitulo = InputBox("Título do Relatório", ConfigSistema1.Default.NomeEmpresa, "Lista de Grupos da Agenda");
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            try
            {
                ...
                int NumerorRegistros = GRUPOAGENDAColl.Count;

                while (IndexRegistro < GRUPOAGENDAColl.Count)
                {
                    config.PosicaoDaLinha = config.MargemSuperior + (config.LinhaAtual * config.FonteNormal.GetHeight(e.Graphics));
                    e.Graphics.DrawString(GRUPOAGENDAColl[IndexRegistro].IDGRUPOAGENDA.ToString(), config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha);
                    e.Graphics.DrawString(Util.LimiterText(GRUPOAGENDAColl[IndexRegistro].NOMEGRUPOAGENDA, 40), config.FonteConteudo, Brushes.Black, config.MargemEsquerda + 50, config.PosicaoDaLinha);
                    e.Graphics.DrawString(Util.LimiterText(GRUPOAGENDAColl[IndexRegistro].OBSERVACAO, 100), config.FonteConteudo, Brushes.Black, config.MargemEsquerda + 250, config.PosicaoDaLinha);
                ...
                if (IndexRegistro < GRUPOAGENDAColl.Count)
                    e.HasMorePages = true;
                else
                {
                    e.Graphics.DrawString("", config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha + 50);
                    e.Graphics.DrawString("Total da pesquisa: " + GRUPOAGENDAColl.Count, config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha + 50);
        ...
        private void FrmGrupoAgenda_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void DataGriewDados_KeyDown(object sender, KeyEventArgs e)
        {
            if (GRUPOAGENDAColl.Count > 0)
            {
                ...
                int CodigoSelect = Convert.ToInt32(GRUPOAGENDAColl[indice].IDGRUPOAGENDA);
                ...
                else if (e.KeyCode == Keys.Delete)
                {
                    ...
                    if (dr == DialogResult.Yes)
                    {
                        try
                        {
                            GRUPOAGENDAPP.Delete(CodigoSelect);
                            Util.ExibirMSg(ConfigMessage.Default.MsgDelete2, "Blue");
                            GetAllGrupoAgenda();
                        }
                        catch (Exception)
