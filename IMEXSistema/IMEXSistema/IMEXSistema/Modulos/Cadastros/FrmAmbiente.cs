...
namespace BmsSoftware.Modulos.Cadastros
{
    public partial class FrmAmbiente : Form
    {
        ...
        public int _IDAMBIENTE = -1;
        public AMBIENTEEntity Entity
        {
            get
            {
                string NOME = txtNome.Text;
                string OBSERVACAO = txtObservacao.Text;
                return new AMBIENTEEntity(_IDAMBIENTE, NOME, OBSERVACAO);
            }
            set
            {
        ...
        private void Grava()
        {
            try
            {
                if (Validacoes())
                {
                    _IDAMBIENTE = AMBIENTEP.Save(Entity);
                    GetAllRegistro();
        ...
        private void GetAllRegistro()
        {
            ...
            DataGriewDados.DataSource = AMBIENTEColl;

            lblTotalPesquisa.Text = AMBIENTEColl.Count.ToString();
        }
        ...
        private void FrmTipoRegiao_Load(object sender, EventArgs e)
        {
            ...
            if (_IDAMBIENTE != -1)
                Entity = AMBIENTEP.Read(_IDAMBIENTE);
        ...
        private void DataGriewDados_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if (AMBIENTEColl.Count > 0)
            {
                ...
                if (rowindex != -1)
                {
                    int CodigoSelect = Convert.ToInt32(AMBIENTEColl[rowindex].IDAMBIENTE);

                    Entity = AMBIENTEP.Read(CodigoSelect);
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            try
            {
                ...
                int NumerorRegistros = AMBIENTEColl.Count;

                while (IndexRegistro < AMBIENTEColl.Count)
                {
                    config.PosicaoDaLinha = config.MargemSuperior + (config.LinhaAtual * config.FonteNormal.GetHeight(e.Graphics));
                    e.Graphics.DrawString(AMBIENTEColl[IndexRegistro].IDAMBIENTE.ToString(), config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha);
                    e.Graphics.DrawString(Util.LimiterText(AMBIENTEColl[IndexRegistro].NOME, 40), config.FonteConteudo, Brushes.Black, config.MargemEsquerda + 50, config.PosicaoDaLinha);
                    e.Graphics.DrawString(Util.LimiterText(AMBIENTEColl[IndexRegistro].OBSERVACAO, 100), config.FonteConteudo, Brushes.Black, config.MargemEsquerda + 250, config.PosicaoDaLinha);
                ...
                if (IndexRegistro < AMBIENTEColl.Count)
                    e.HasMorePages = true;
                else
                {
                    e.Graphics.DrawString("", config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha + 50);
                    e.Graphics.DrawString("Total da pesquisa: " + AMBIENTEColl.Count, config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha + 50);
        ...
        private void DataGriewDados_KeyDown(object sender, KeyEventArgs e)
        {
            if (AMBIENTEColl.Count > 0)
            {
                ...
                int CodigoSelect = Convert.ToInt32(AMBIENTEColl[indice].IDAMBIENTE);
