...
{
    public partial class FrmProfissaoRamoAtividade : Form
    {
        ...
        public FrmProfissaoRamoAtividade()
        {
            InitializeComponent();
        ...
        int _IDPROFISSAORAMOATIVIDADE = -1;

        public PROFISSAORAMOATIVIDADEEntity Entity
        {
            get
            {
                string NOME = txtNome.Text;
                string OBSERVACAO = txtObservacao.Text;
                return new PROFISSAORAMOATIVIDADEEntity(_IDPROFISSAORAMOATIVIDADE, NOME, OBSERVACAO);
            }
            set
            {
                ...
                if (value != null)
                {
                    _IDPROFISSAORAMOATIVIDADE = value.IDPROFISSAORAMOATIVIDADE;
        ...
        private void Grava()
        {
            try
            {
                if (Validacoes())
                {
                    _IDPROFISSAORAMOATIVIDADE = PROFISSAORAMOATIVIDADEP.Save(Entity);
        ...
        private void GetAllProfissaoAtividade()
        {
            PROFISSAORAMOATIVIDADEColl = PROFISSAORAMOATIVIDADEP.ReadCollectionByParameter(null, "NOME");
            DataGriewDados.AutoGenerateColumns = false;
            DataGriewDados.DataSource = PROFISSAORAMOATIVIDADEColl;

            lblTotalPesquisa.Text = PROFISSAORAMOATIVIDADEColl.Count.ToString();
        }
        ...
        {
            Entity = null;
            tabControlProfissaoRamoAtividade.SelectTab(0);
            txtNome.Focus();
        }
        ...
        private void Delete()
        {
            if (_IDPROFISSAORAMOATIVIDADE == -1)
            {
                MessageBox.Show(ConfigMessage.Default.MsgSelecRegistro);
                tabControlProfissaoRamoAtividade.SelectTab(1);
            }
            else
            {
                ...
                if (dr == DialogResult.Yes)
                {
                    try
                    {
                        PROFISSAORAMOATIVIDADEP.Delete(_IDPROFISSAORAMOATIVIDADE);
        ...
        {
            if (PROFISSAORAMOATIVIDADEColl.Count > 0)
            {
                int rowindex = e.RowIndex;
                if (rowindex != -1)
                {
                    int CodigoSelect = Convert.ToInt32(PROFISSAORAMOATIVIDADEColl[rowindex].IDPROFISSAORAMOATIVIDADE);

                    Entity = PROFISSAORAMOATIVIDADEP.Read(CodigoSelect);
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            try
            {
                ...
                int NumerorRegistros = PROFISSAORAMOATIVIDADEColl.Count;

                while (IndexRegistro < PROFISSAORAMOATIVIDADEColl.Count)
                {
                    config.PosicaoDaLinha = config.MargemSuperior + (config.LinhaAtual * config.FonteNormal.GetHeight(e.Graphics));
                    e.Graphics.DrawString(PROFISSAORAMOATIVIDADEColl[IndexRegistro].IDPROFISSAORAMOATIVIDADE.ToString(), config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha);
                    e.Graphics.DrawString(Util.LimiterText(PROFISSAORAMOATIVIDADEColl[IndexRegistro].NOME, 40), config.FonteConteudo, Brushes.Black, config.MargemEsquerda + 50, config.PosicaoDaLinha);
                ...
                if (IndexRegistro < PROFISSAORAMOATIVIDADEColl.Count)
                    e.HasMorePages = true;
                else
                {
                    e.Graphics.DrawString("", config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha + 50);
                    e.Graphics.DrawString("Total da pesquisa: " + PROFISSAORAMOATIVIDADEColl.Count, config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha + 50);
        ...
        {
            RelatorioTitulo = InputBox("Título do Relatório", ConfigSistema1.Default.NomeEmpresa, "Lista de Profissão/Ramo de Atividade");
        ...
        private void FrmProfissaoRamoAtividade_FormClosing(object sender, FormClosingEventArgs e)
        {
            e.Cancel = false;
        }

        private void FrmProfissaoRamoAtividade_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void DataGriewDados_KeyDown(object sender, KeyEventArgs e)
        {
            if (PROFISSAORAMOATIVIDADEColl.Count > 0)
            {
                ...
                int CodigoSelect = Convert.ToInt32(PROFISSAORAMOATIVIDADEColl[indice].IDPROFISSAORAMOATIVIDADE);
                ...
                else if (e.KeyCode == Keys.Delete)
                {
                    ...
                    if (dr == DialogResult.Yes)
                    {
                        try
                        {
                            PROFISSAORAMOATIVIDADEP.Delete(CodigoSelect);
                            Util.ExibirMSg(ConfigMessage.Default.MsgDelete2, "Blue");
                            GetAllProfissaoAtividade();
                        }
                        catch (Exception)
