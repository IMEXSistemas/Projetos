...
namespace BmsSoftware.Modulos.Cadastros
{
    public partial class FrmMensagem : Form
    {
        ...
        int _IDMENSAGEM = -1;
        public MENSAGEMEntity Entity
        {
            get
            {
                string NOME = txtNome.Text;
                string MENSAGEM = txtMensagem.Text;
                return new MENSAGEMEntity(_IDMENSAGEM, MENSAGEM, NOME);
            }
            set
            {
        ...
        private void Grava()
        {
            try
            {
                if (Validacoes())
                {
                    _IDMENSAGEM = MENSAGEMP.Save(Entity);
                    GetAllMarca();
        ...
        {
            MENSAGEMColl = MENSAGEMP.ReadCollectionByParameter(null, "NOME");
            DataGriewDados.AutoGenerateColumns = false;
            DataGriewDados.DataSource = MENSAGEMColl;

            lblTotalPesquisa.Text = MENSAGEMColl.Count.ToString();
        }
        ...
        {
            if (MENSAGEMColl.Count > 0) 
            {
                int rowindex = e.RowIndex;
                if (rowindex != -1)
                {
                    int CodigoSelect = Convert.ToInt32(MENSAGEMColl[rowindex].IDMENSAGEM);

                    Entity = MENSAGEMP.Read(CodigoSelect);
        ...
        {
            using (FrmRelatMensagem frm = new FrmRelatMensagem())
            {
                frm.Mensagem = txtMensagem.Text;
                frm.NomeMensagem = txtNome.Text;
                frm.ShowDialog();
            }
        }
        ...
        {
            RelatorioTitulo = InputBox("Título do Relatório", ConfigSistema1.Default.NomeEmpresa, "Lista de Mensagens");
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            try
            {
                ...
                e.Graphics.DrawString("Mensagem", config.FonteNegrito, Brushes.Black, config.MargemEsquerda + 250, 170);
                ...
                int NumerorRegistros = MENSAGEMColl.Count;

                while (IndexRegistro < MENSAGEMColl.Count)
                {
                    config.PosicaoDaLinha = config.MargemSuperior + (config.LinhaAtual * config.FonteNormal.GetHeight(e.Graphics));
                    e.Graphics.DrawString(MENSAGEMColl[IndexRegistro].IDMENSAGEM.ToString(), config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha);
                    e.Graphics.DrawString(Util.LimiterText(MENSAGEMColl[IndexRegistro].NOME, 40), config.FonteConteudo, Brushes.Black, config.MargemEsquerda + 50, config.PosicaoDaLinha);
                    e.Graphics.DrawString(Util.LimiterText(MENSAGEMColl[IndexRegistro].MENSAGEM, 100), config.FonteConteudo, Brushes.Black, config.MargemEsquerda + 250, config.PosicaoDaLinha);
                ...
                if (IndexRegistro < MENSAGEMColl.Count)
                    e.HasMorePages = true;
                else
                {
                    e.Graphics.DrawString("", config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha + 50);
                    e.Graphics.DrawString("Total da pesquisa: " + MENSAGEMColl.Count, config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha + 50);
        ...
        private void DataGriewDados_KeyDown(object sender, KeyEventArgs e)
        {
            if (MENSAGEMColl.Count > 0)
            {
                ...
                int CodigoSelect = Convert.ToInt32(MENSAGEMColl[indice].IDMENSAGEM);
