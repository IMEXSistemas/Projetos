...
namespace BmsSoftware.Modulos.Cadastros
{
    public partial class FrmDepartamento : Form
    {
        ...
        DEPARTAMENTOCollection DEPARTAMENTOColl = new DEPARTAMENTOCollection();
        ...
        public DEPARTAMENTOEntity Entity
        {
            get
            {
                string NOMEDEPARTAMENTO = txtNome.Text;
                string OBSERVACAO = txtObservacao.Text;
                return new DEPARTAMENTOEntity(_IDDEPARTAMENTO, NOMEDEPARTAMENTO, OBSERVACAO);
            }
            set
            {
                ...
                if (value != null)
                {
                    ...
                    txtNome.Text = value.NOMEDEPARTAMENTO;
        ...
        private void Grava()
        {
            try
            {
                if (Validacoes())
                {
                    _IDDEPARTAMENTO = DEPARTAMENTOP.Save(Entity);
                    GetAllDepartamento();
        ...
        private void GetAllDepartamento()
        {
            DEPARTAMENTOColl = DEPARTAMENTOP.ReadCollectionByParameter(null, "NOMEDEPARTAMENTO");
            DataGriewDados.AutoGenerateColumns = false;
            DataGriewDados.DataSource = DEPARTAMENTOColl;

            lblTotalPesquisa.Text = DEPARTAMENTOColl.Count.ToString();
        }
        ...
        private void Delete()
        {
            ...
            else
            {
                ...
                if (dr == DialogResult.Yes)
                {
                    try
                    {
                        DEPARTAMENTOP.Delete(_IDDEPARTAMENTO);
        ...
        {
            if (DEPARTAMENTOColl.Count > 0)
            {
                int rowindex = e.RowIndex;
                if (rowindex != -1)
                {
                    int CodigoSelect = Convert.ToInt32(DEPARTAMENTOColl[rowindex].IDDEPARTAMENTO);

                    Entity = DEPARTAMENTOP.Read(CodigoSelect);
        ...
        {
            RelatorioTitulo = InputBox("Título do Relatório", ConfigSistema1.Default.NomeEmpresa, "Lista de Departamento");
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            try
            {
                ...
                int NumerorRegistros = DEPARTAMENTOColl.Count;

                while (IndexRegistro < DEPARTAMENTOColl.Count)
                {
                    config.PosicaoDaLinha = config.MargemSuperior + (config.LinhaAtual * config.FonteNormal.GetHeight(e.Graphics));
                    e.Graphics.DrawString(DEPARTAMENTOColl[IndexRegistro].IDDEPARTAMENTO.ToString(), config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha);
                    e.Graphics.DrawString(Util.LimiterText(DEPARTAMENTOColl[IndexRegistro].NOMEDEPARTAMENTO, 40), config.FonteConteudo, Brushes.Black, config.MargemEsquerda + 50, config.PosicaoDaLinha);
                    e.Graphics.DrawString(Util.LimiterText(DEPARTAMENTOColl[IndexRegistro].OBSERVACAO, 100), config.FonteConteudo, Brushes.Black, config.MargemEsquerda + 250, config.PosicaoDaLinha);
                ...
                if (IndexRegistro < DEPARTAMENTOColl.Count)
                    e.HasMorePages = true;
                else
                {
                    e.Graphics.DrawString("", config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha + 50);
                    e.Graphics.DrawString("Total da pesquisa: " + DEPARTAMENTOColl.Count, config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha + 50);
        ...
        private void DataGriewDados_KeyDown(object sender, KeyEventArgs e)
        {
            if (DEPARTAMENTOColl.Count > 0)
            {
                ...
                int CodigoSelect = Convert.ToInt32(DEPARTAMENTOColl[indice].IDDEPARTAMENTO);
                ...
                else if (e.KeyCode == Keys.Delete)
                {
                    ...
                    if (dr == DialogResult.Yes)
                    {
                        try
                        {
                            DEPARTAMENTOP.Delete(CodigoSelect);
                            Util.ExibirMSg(ConfigMessage.Default.MsgDelete2, "Blue");
                            GetAllDepartamento();
                        }
                        catch (Exception)
