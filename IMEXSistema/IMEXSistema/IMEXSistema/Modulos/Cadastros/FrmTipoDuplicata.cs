...
{
    public partial class FrmTipoDuplicata : Form
    {
       ...
       public  int _IDTIPODUPLICATA = -1;

        public FrmTipoDuplicata()
        {
            InitializeComponent();
        ...
        public TIPODUPLICATAEntity Entity
        {
            get
            {
                string NOME = txtNome.Text;
                string OBSERVACAO = txtObservacao.Text;
                return new TIPODUPLICATAEntity(_IDTIPODUPLICATA, NOME, OBSERVACAO);
            }
            set
            {
                ...
                if (value != null)
                {
                    _IDTIPODUPLICATA = value.IDTIPODUPLICATA;
        ...
        private void Grava()
        {
            try
            {
                if (Validacoes())
                {
                   _IDTIPODUPLICATA  = TIPODUPLICATAP.Save(Entity);
                    GetAllTipoDuplicata();
        ...
        private void GetAllTipoDuplicata()
        {
            TIPODUPLICATAColl = TIPODUPLICATAP.ReadCollectionByParameter(null, "NOME");
            DataGriewDados.AutoGenerateColumns = false;
            DataGriewDados.DataSource = TIPODUPLICATAColl;

            lblTotalPesquisa.Text = TIPODUPLICATAColl.Count.ToString();
        }
        ...
        {
            RelatorioTitulo = InputBox("Título do Relatório", ConfigSistema1.Default.NomeEmpresa, "Lista de Tipo Duplicata");
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            try
            {
                ...
                int NumerorRegistros = TIPODUPLICATAColl.Count;

                while (IndexRegistro < TIPODUPLICATAColl.Count)
                {
                    config.PosicaoDaLinha = config.MargemSuperior + (config.LinhaAtual * config.FonteNormal.GetHeight(e.Graphics));
                    e.Graphics.DrawString(TIPODUPLICATAColl[IndexRegistro].IDTIPODUPLICATA.ToString(), config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha);
                    e.Graphics.DrawString(Util.LimiterText(TIPODUPLICATAColl[IndexRegistro].NOME, 40), config.FonteConteudo, Brushes.Black, config.MargemEsquerda + 50, config.PosicaoDaLinha);
                    e.Graphics.DrawString(Util.LimiterText(TIPODUPLICATAColl[IndexRegistro].OBSERVACAO, 100), config.FonteConteudo, Brushes.Black, config.MargemEsquerda + 250, config.PosicaoDaLinha);
                ...
                if (IndexRegistro < TIPODUPLICATAColl.Count)
                    e.HasMorePages = true;
                else
                {
                    e.Graphics.DrawString("", config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha + 50);
                    e.Graphics.DrawString("Total da pesquisa: " + TIPODUPLICATAColl.Count, config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha + 50);
