...
{
    public partial class FrmCodMovEstoque : Form
    {
        Utility Util = new Utility();

        CODMOVESTOQUEProvider CODMOVESTOQUEP = new CODMOVESTOQUEProvider();
        CODMOVESTOQUECollection CODMOVESTOQUEColl = new CODMOVESTOQUECollection();

        int _IDCODMOVESTOQUE = -1;

        public FrmCodMovEstoque()
        {
            InitializeComponent();
        ...
        public CODMOVESTOQUEEntity Entity
        {
            get
            {
                string NOME = txtNome.Text;
                string OBSERVACAO = txtObservacao.Text;
                string CODIGO = txtCodMovimentação.Text;
                return new CODMOVESTOQUEEntity(_IDCODMOVESTOQUE, CODIGO, NOME, OBSERVACAO);
            }
            set
            {
                ...
                if (value != null)
                {
                    _IDCODMOVESTOQUE = value.IDCODMOVESTOQUE;
                    txtNome.Text = value.NOME;
                    txtCodMovimentação.Text = value.CODIGO;
                ...
                else
                {
                    ...
                    txtCodMovimentação.Text = string.Empty;
        ...
        private void Grava()
        {
            try
            {
                if (Validacoes())
                {
                    _IDCODMOVESTOQUE = CODMOVESTOQUEP.Save(Entity);
                    errorProvider1.Clear();
                    GetAllMovEstoque();
        ...
        private void GetAllMovEstoque()
        {
            CODMOVESTOQUEColl = CODMOVESTOQUEP.ReadCollectionByParameter(null, "NOME");
            DataGriewDados.AutoGenerateColumns = false;
            DataGriewDados.DataSource = CODMOVESTOQUEColl;

            lblTotalPesquisa.Text = CODMOVESTOQUEColl.Count.ToString();
        }        
        ...
        private Boolean Validacoes()
        {
            ...
            else if (txtCodMovimentação.Text.Trim().Length == 0)
            {
                errorProvider1.SetError(txtCodMovimentação, ConfigMessage.Default.CampoObrigatorio);
                Util.ExibirMSg(ConfigMessage.Default.CampoObrigatorio2, "Red");
                result = false;
            }
            else
        ...
        {
            Entity = null;
            tabControlCodMovEstoque.SelectTab(0);
            txtNome.Focus();
        }
        ...
        private void Delete()
        {
            if (_IDCODMOVESTOQUE == -1)
            {
                MessageBox.Show(ConfigMessage.Default.MsgSelecRegistro);
                tabControlCodMovEstoque.SelectTab(1);
            }
            else
            {
                ...
                if (dr == DialogResult.Yes)
                {
                    try
                    {
                        CODMOVESTOQUEP.Delete(_IDCODMOVESTOQUE);
        ...
        {
            if (CODMOVESTOQUEColl.Count > 0)
            {
                int rowindex = e.RowIndex;
                if (rowindex != -1)
                {
                    int CodigoSelect = Convert.ToInt32(CODMOVESTOQUEColl[rowindex].IDCODMOVESTOQUE);

                    Entity = CODMOVESTOQUEP.Read(CodigoSelect);
        ...
        {
            RelatorioTitulo = InputBox("Título do Relatório", ConfigSistema1.Default.NomeEmpresa, "Lista de Códigos para Movimentação de Estoque");
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            try
            {
                ...
                int NumerorRegistros = CODMOVESTOQUEColl.Count;

                while (IndexRegistro < CODMOVESTOQUEColl.Count)
                {
                    config.PosicaoDaLinha = config.MargemSuperior + (config.LinhaAtual * config.FonteNormal.GetHeight(e.Graphics));
                    e.Graphics.DrawString(CODMOVESTOQUEColl[IndexRegistro].CODIGO.ToString(), config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha);
                    e.Graphics.DrawString(Util.LimiterText(CODMOVESTOQUEColl[IndexRegistro].NOME, 40), config.FonteConteudo, Brushes.Black, config.MargemEsquerda + 50, config.PosicaoDaLinha);
                    e.Graphics.DrawString(Util.LimiterText(CODMOVESTOQUEColl[IndexRegistro].OBSERVACAO, 100), config.FonteConteudo, Brushes.Black, config.MargemEsquerda + 250, config.PosicaoDaLinha);
                ...
                if (IndexRegistro < CODMOVESTOQUEColl.Count)
                    e.HasMorePages = true;
                else
                {
                    e.Graphics.DrawString("", config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha + 50);
                    e.Graphics.DrawString("Total da pesquisa: " + CODMOVESTOQUEColl.Count, config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha + 50);
        ...
        private void DataGriewDados_KeyDown(object sender, KeyEventArgs e)
        {
            if (CODMOVESTOQUEColl.Count > 0)
            {
                ...
                int CodigoSelect = Convert.ToInt32(CODMOVESTOQUEColl[indice].IDCODMOVESTOQUE);
                ...
                else if (e.KeyCode == Keys.Delete)
                {
                    ...
                    if (dr == DialogResult.Yes)
                    {
                        try
                        {
                            CODMOVESTOQUEP.Delete(CodigoSelect);
                            Util.ExibirMSg(ConfigMessage.Default.MsgDelete2, "Blue");
                            GetAllMovEstoque();
                        }
                        catch (Exception)
