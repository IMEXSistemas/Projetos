...
namespace BmsSoftware.Modulos.Cadastros
{
    public partial class FrmCor : Form
    {
        ...
        public int _IDCOR = -1;
        public COREntity Entity
        {
            get
            {
                string NOME = txtNome.Text;
                string OBSERVACAO = txtObservacao.Text;
                return new COREntity(_IDCOR, NOME, OBSERVACAO);
            }
            set
            {
        ...
        private void GetAllMarca()
        {
            ...
            DataGriewDados.DataSource = CORColl;

            lblTotalPesquisa.Text = CORColl.Count.ToString();
        }
        ...
        private void DataGriewDados_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if (CORColl.Count > 0)
            {
                ...
                if (rowindex != -1)
                {
                    int CodigoSelect = Convert.ToInt32(CORColl[rowindex].IDCOR);

                    Entity = CORP.Read(CodigoSelect);
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            try
            {
                ...
                int NumerorRegistros = CORColl.Count;

                while (IndexRegistro < CORColl.Count)
                {
                    config.PosicaoDaLinha = config.MargemSuperior + (config.LinhaAtual * config.FonteNormal.GetHeight(e.Graphics));
                    e.Graphics.DrawString(CORColl[IndexRegistro].IDCOR.ToString(), config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha);
                    e.Graphics.DrawString(Util.LimiterText(CORColl[IndexRegistro].NOME, 40), config.FonteConteudo, Brushes.Black, config.MargemEsquerda + 50, config.PosicaoDaLinha);
                    e.Graphics.DrawString(Util.LimiterText(CORColl[IndexRegistro].OBSERVACAO, 100), config.FonteConteudo, Brushes.Black, config.MargemEsquerda + 250, config.PosicaoDaLinha);
                ...
                if (IndexRegistro < CORColl.Count)
                    e.HasMorePages = true;
                else
                {
                    e.Graphics.DrawString("", config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha + 50);
                    e.Graphics.DrawString("Total da pesquisa: " + CORColl.Count, config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha + 50);
        ...
        private void DataGriewDados_KeyDown(object sender, KeyEventArgs e)
        {
            if (CORColl.Count > 0)
            {
                ...
                int CodigoSelect = Convert.ToInt32(CORColl[indice].IDCOR);
