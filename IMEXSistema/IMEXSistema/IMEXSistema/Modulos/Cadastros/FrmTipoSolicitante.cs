...
{
    public partial class FrmTipoSolicitante : Form
    {
        Utility Util = new Utility();
        TIPOSOLICITANTEProvider TIPOSOLICITANTEP = new TIPOSOLICITANTEProvider();
        TIPOSOLICITANTECollection TIPOSOLICITANTEColl = new TIPOSOLICITANTECollection();
        int _IDTIPOSOLICITANTE = -1;

        public FrmTipoSolicitante()
        {
            InitializeComponent();
        ...
        public TIPOSOLICITANTEEntity Entity
        {
            get
            {
                string NOME = txtNome.Text;
                string OBSERVACAO = txtObservacao.Text;
                return new TIPOSOLICITANTEEntity(_IDTIPOSOLICITANTE, NOME, OBSERVACAO);
            }
            set
            {
                ...
                if (value != null)
                {
                    _IDTIPOSOLICITANTE = value.IDTIPOSOLICITANTE;
        ...
        private void Grava()
        {
            try
            {
                if (Validacoes())
                {
                    _IDTIPOSOLICITANTE = TIPOSOLICITANTEP.Save(Entity);
                    GetAllTipoSolicitante();
        ...
        private void GetAllTipoSolicitante()
        {
            TIPOSOLICITANTEColl = TIPOSOLICITANTEP.ReadCollectionByParameter(null, "NOME");
            DataGriewDados.AutoGenerateColumns = false;
            DataGriewDados.DataSource = TIPOSOLICITANTEColl;

            lblTotalPesquisa.Text = TIPOSOLICITANTEColl.Count.ToString();
        }
        ...
        {
            Entity = null;
            tabControlTipoSolic.SelectTab(0);
            txtNome.Focus();
        }
        ...
        private void Delete()
        {
            if (_IDTIPOSOLICITANTE == -1)
            {
                MessageBox.Show(ConfigMessage.Default.MsgSelecRegistro);
                tabControlTipoSolic.SelectTab(1);
            }
            else
            {
                ...
                if (dr == DialogResult.Yes)
                {
                    try
                    {
                        TIPOSOLICITANTEP.Delete(_IDTIPOSOLICITANTE);
        ...
        {
            if (TIPOSOLICITANTEColl.Count > 0)
            {
                int rowindex = e.RowIndex;
                if (rowindex != -1)
                {
                    int CodigoSelect = Convert.ToInt32(TIPOSOLICITANTEColl[rowindex].IDTIPOSOLICITANTE);

                    Entity = TIPOSOLICITANTEP.Read(CodigoSelect);
        ...
        {
            RelatorioTitulo = InputBox("Título do Relatório", ConfigSistema1.Default.NomeEmpresa, "Lista de Tipo de Solicitante");
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            try
            {
                ...
                int NumerorRegistros = TIPOSOLICITANTEColl.Count;

                while (IndexRegistro < TIPOSOLICITANTEColl.Count)
                {
                    config.PosicaoDaLinha = config.MargemSuperior + (config.LinhaAtual * config.FonteNormal.GetHeight(e.Graphics));
                    e.Graphics.DrawString(TIPOSOLICITANTEColl[IndexRegistro].IDTIPOSOLICITANTE.ToString(), config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha);
                    e.Graphics.DrawString(Util.LimiterText(TIPOSOLICITANTEColl[IndexRegistro].NOME, 40), config.FonteConteudo, Brushes.Black, config.MargemEsquerda + 50, config.PosicaoDaLinha);
                    e.Graphics.DrawString(Util.LimiterText(TIPOSOLICITANTEColl[IndexRegistro].OBSERVACAO, 100), config.FonteConteudo, Brushes.Black, config.MargemEsquerda + 250, config.PosicaoDaLinha);
                ...
                if (IndexRegistro < TIPOSOLICITANTEColl.Count)
                    e.HasMorePages = true;
                else
                {
                    e.Graphics.DrawString("", config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha + 50);
                    e.Graphics.DrawString("Total da pesquisa: " + TIPOSOLICITANTEColl.Count, config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha + 50);
        ...
        private void FrmTipoSolicitante_FormClosing(object sender, FormClosingEventArgs e)
        {
            e.Cancel = false;
        }

        private void FrmTipoSolicitante_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void DataGriewDados_KeyDown(object sender, KeyEventArgs e)
        {
            if (TIPOSOLICITANTEColl.Count > 0)
            {
                ...
                int CodigoSelect = Convert.ToInt32(TIPOSOLICITANTEColl[indice].IDTIPOSOLICITANTE);
                ...
                else if (e.KeyCode == Keys.Delete)
                {
                    ...
                    if (dr == DialogResult.Yes)
                    {
                        try
                        {
                            TIPOSOLICITANTEP.Delete(CodigoSelect);
                            Util.ExibirMSg(ConfigMessage.Default.MsgDelete2, "Blue");
                            GetAllTipoSolicitante();
                        }
                        catch (Exception)
