...
namespace BmsSoftware.Modulos.Cadastros
{
    public partial class FrmCentroCustos : Form
    {
        ...
        public int _IDCENTROCUSTOS = -1;
        public CENTROCUSTOSEntity Entity
        {
            get
            {
                string CENTROCUSTOS = txtCentroCustos.Text;
                string DESCRICAO = txtDescricao.Text;
                string OBSERVACAO = txtObservacao.Text;
                return new CENTROCUSTOSEntity(_IDCENTROCUSTOS, CENTROCUSTOS, DESCRICAO,
                                               OBSERVACAO);
            }
            set
            {
                ...
                if (value != null)
                {
                    _IDCENTROCUSTOS = value.IDCENTROCUSTOS;
                    txtCentroCustos.Text = value.CENTROCUSTO;
                ...
                {
                    _IDCENTROCUSTOS =-1;
                    txtCentroCustos.Text = string.Empty;
        ...
        private void Grava()
        {
            try
            {
                if (Validacoes())
                {
                    _IDCENTROCUSTOS = CENTROCUSTOSP.Save(Entity);
                    GetAllCentroCusto();
        ...
        private void GetAllCentroCusto()
        {
            CENTROCUSTOSColl = CENTROCUSTOSP.ReadCollectionByParameter(null, "CENTROCUSTO");
            DataGriewDados.AutoGenerateColumns = false;
            DataGriewDados.DataSource = CENTROCUSTOSColl;

            lblTotalPesquisa.Text = CENTROCUSTOSColl.Count.ToString();
        }
        ...
        private Boolean Validacoes()
        {
            ...
            errorProvider1.Clear();
            if (txtCentroCustos.Text.Trim().Length == 0)
            {
                errorProvider1.SetError(txtCentroCustos, ConfigMessage.Default.CampoObrigatorio);
                ...
                if (txtDescricao.Text.Trim().Length == 0)
            {
                ...
                errorProvider1.SetError(txtCentroCustos, "");
        ...
        private void FrmTipoRegiao_Load(object sender, EventArgs e)
        {
            ...
            if (_IDCENTROCUSTOS != -1)
                Entity = CENTROCUSTOSP.Read(_IDCENTROCUSTOS);
        ...
        {
            Entity = null;
            tabControlCentroCusto.SelectTab(0);
            txtCentroCustos.Focus();
        }
        ...
        private void Delete()
        {
            if (_IDCENTROCUSTOS == -1)
            {
                ...
                tabControlCentroCusto.SelectTab(1);
            }
            else
            {
                ...
                if (dr == DialogResult.Yes)
                {
                    try
                    {
                        CENTROCUSTOSP.Delete(_IDCENTROCUSTOS);
        ...
        private void DataGriewDados_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if (CENTROCUSTOSColl.Count > 0)
            {
                ...
                if (rowindex != -1)
                {
                    int CodigoSelect = Convert.ToInt32(CENTROCUSTOSColl[rowindex].IDCENTROCUSTOS);

                    Entity = CENTROCUSTOSP.Read(CodigoSelect);
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            try
            {
                ...
                e.Graphics.DrawString("Centro de Custo", config.FonteNegrito, Brushes.Black, config.MargemEsquerda + 50, 170);
                e.Graphics.DrawString("Descrição", config.FonteNegrito, Brushes.Black, config.MargemEsquerda + 250, 170);
                ...
                int NumerorRegistros = CENTROCUSTOSColl.Count;

                while (IndexRegistro < CENTROCUSTOSColl.Count)
                {
                    config.PosicaoDaLinha = config.MargemSuperior + (config.LinhaAtual * config.FonteNormal.GetHeight(e.Graphics));
                    e.Graphics.DrawString(CENTROCUSTOSColl[IndexRegistro].IDCENTROCUSTOS.ToString(), config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha);
                    e.Graphics.DrawString(Util.LimiterText(CENTROCUSTOSColl[IndexRegistro].CENTROCUSTO, 40), config.FonteConteudo, Brushes.Black, config.MargemEsquerda + 50, config.PosicaoDaLinha);
                    e.Graphics.DrawString(Util.LimiterText(CENTROCUSTOSColl[IndexRegistro].DESCRICAO, 100), config.FonteConteudo, Brushes.Black, config.MargemEsquerda + 250, config.PosicaoDaLinha);
                ...
                if (IndexRegistro < CENTROCUSTOSColl.Count)
                    e.HasMorePages = true;
                else
                {
                    e.Graphics.DrawString("", config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha + 50);
                    e.Graphics.DrawString("Total da pesquisa: " + CENTROCUSTOSColl.Count, config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha + 50);
        ...
        private void DataGriewDados_KeyDown(object sender, KeyEventArgs e)
        {
            if (CENTROCUSTOSColl.Count > 0)
            {
                ...
                int CodigoSelect = Convert.ToInt32(CENTROCUSTOSColl[indice].IDCENTROCUSTOS);
                ...
                else if (e.KeyCode == Keys.Delete)
                {
                    ...
                    if (dr == DialogResult.Yes)
                    {
                        try
                        {
                            CENTROCUSTOSP.Delete(CodigoSelect);
                            Util.ExibirMSg(ConfigMessage.Default.MsgDelete2, "Blue");
                            GetAllCentroCusto();
                        }
                        catch (Exception)
