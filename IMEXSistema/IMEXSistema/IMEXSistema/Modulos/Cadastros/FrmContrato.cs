...
namespace BmsSoftware.Modulos.Cadastros
{
    public partial class FrmContrato : Form
    {
        ...
        public int _IDCONTRATO = -1;
        public CONTRATOEntity Entity
        {
            get
            {
                string NOME = txtNome.Text;
                string DESCRICAO = txtDescricao.Text;
                string FLAGPRINCIPAL = chkPrincipal.Checked ? "S" : "N";
                return new CONTRATOEntity(_IDCONTRATO, NOME, DESCRICAO, FLAGPRINCIPAL);
            }
            set
            {
                ...
                if (value != null)
                {
                    ...
                    txtDescricao.Text = value.DESCRICAO.TrimEnd();
                    chkPrincipal.Checked = value.FLAGPRINCIPAL.TrimEnd() == "S" ? true : false;
        ...
        private void Grava()
        {
            try
            {
                if (Validacoes())
                {
                  _IDCONTRATO = CONTRATOP.Save(Entity);
                    GetAllMarca();
        ...
        private void GetAllMarca()
        {
            ...
            DataGriewDados.DataSource = CONTRATOColl;

            lblTotalPesquisa.Text = CONTRATOColl.Count.ToString();
        }
        ...
        private void FrmTipoRegiao_Load(object sender, EventArgs e)
        {
            ...
            if (_IDCONTRATO != -1)
                Entity = CONTRATOP.Read(_IDCONTRATO);
        ...
        {
            if (CONTRATOColl.Count > 0)
            {
                int rowindex = e.RowIndex;
                if (rowindex != -1)
                {
                    int CodigoSelect = Convert.ToInt32(CONTRATOColl[rowindex].IDCONTRATO);

                    Entity = CONTRATOP.Read(CodigoSelect);
        ...
        private void ImprimirListaGeral()
        {
            ...
            {
                MessageBox.Show(ConfigMessage.Default.MsgSelecRegistro);
                tabControlMarca.SelectTab(2);
            }
            else
            {
               CONTRATOP.Save(Entity);
        ...
        private void ImprimirReceitaReport()
        {
            ...
            {
                RowRelatorio.Clear();
                RowRelatorio.Add(new RowsFiltro("IDCONTRATO", "System.Int32", "=", _IDCONTRATO.ToString()));
        ...
        private void FrmMarca_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            {
                if (this.ActiveControl.Name != "txtDescricao")
        ...
        private void DataGriewDados_KeyDown(object sender, KeyEventArgs e)
        {
            if (CONTRATOColl.Count > 0)
            {
                ...
                int CodigoSelect = Convert.ToInt32(CONTRATOColl[indice].IDCONTRATO);
