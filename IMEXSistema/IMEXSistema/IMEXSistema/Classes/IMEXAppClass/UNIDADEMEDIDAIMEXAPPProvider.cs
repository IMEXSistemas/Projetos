...
{
    public partial class UNIDADEMEDIDAIMEXAPPProvider
    {
        CONFISISTEMAProvider CONFISISTEMAP = new CONFISISTEMAProvider();
        CONFISISTEMAEntity CONFISISTEMATy = new CONFISISTEMAEntity();

        IList<UNIDADEMEDIDAIMEXAPPEntity> UNIDADEMEDIDAIMEXAPPColl;

        Utility Util = new Utility();

        public async void Save(UNIDADEMEDIDAIMEXAPPEntity Entity)
        {
            try
            {
                ...
                Entity.NCASASDECIMAIS = 2;
                Entity.XUSERINCLUSAO = "";
                Entity.STATIVO = true;
        ...
        public int GetID(int CodRegistro)
        {
            ...
            try
            {
                ...
                CONFISISTEMATy = CONFISISTEMAP.Read(1);
                string token = CONFISISTEMATy.TOKENIMEXAPP.Trim();
                string URI = BmsSoftware.Modulos.IMEXApp.UrlIMEXApp.Default.GetRegistrosUnidadeMedida;
                ...
                using (HttpClient client = new HttpClient())
                {
                    ...
                    UNIDADEMEDIDAIMEXAPPColl = DeserializeToList<UNIDADEMEDIDAIMEXAPPEntity>(jsonString);
                }
        ...
        public int BuscaID(int IDRegistro)
        {
            ...
            try
            {
                foreach (var item in UNIDADEMEDIDAIMEXAPPColl)
                {
                    if (item.XMEUID == IDRegistro.ToString())
                    {
                        result = Convert.ToInt32(item.IDUNIDADEMEDIDA);
                        break;
                    }
                }
