...
{
    public partial class RAMOATIVIDADEIMEXAPPProvider
    {
        CONFISISTEMAProvider CONFISISTEMAP = new CONFISISTEMAProvider();
        CONFISISTEMAEntity CONFISISTEMATy = new CONFISISTEMAEntity();

        IList<RAMOATIVIDADEIMEXAPPEntity> RAMOATIVIDADEIMEXAPPColl;

        Utility Util = new Utility();

        public async void Save(RAMOATIVIDADEIMEXAPPEntity Entity)
        {
            try
            {
                ...
                string URI = BmsSoftware.Modulos.IMEXApp.UrlIMEXApp.Default.PostRamoAtividade;

                Entity.IDRAMOATIVIDADE = null;
        ...
        public async void Delete(int CodRegistro)
        {
            try
            {
                ...
                CONFISISTEMATy = CONFISISTEMAP.Read(1);
                string token = CONFISISTEMATy.TOKENIMEXAPP.Trim();
                string URI = BmsSoftware.Modulos.IMEXApp.UrlIMEXApp.Default.DeleteRegistrosRamoAtividade + token + "/";
        ...
        public int GetID(int CodRegistro)
        {
            ...
            try
            {
                ...
                CONFISISTEMATy = CONFISISTEMAP.Read(1);
                string token = CONFISISTEMATy.TOKENIMEXAPP.Trim();
                string URI = BmsSoftware.Modulos.IMEXApp.UrlIMEXApp.Default.GetRegistrosRamoAtividade;
                ...
                using (HttpClient client = new HttpClient())
                {
                    ...
                    RAMOATIVIDADEIMEXAPPColl = DeserializeToList<RAMOATIVIDADEIMEXAPPEntity>(jsonString);
                }
        ...
        public int GetID()
        {
            ...
            try
            {
                ...
                if(RAMOATIVIDADEIMEXAPPColl.Count > 0)
                    Result = Convert.ToInt32(RAMOATIVIDADEIMEXAPPColl[0].IDRAMOATIVIDADE);
        ...
        public int BuscaID(int IDRegistro)
        {
            ...
            try
            {
                foreach (var item in RAMOATIVIDADEIMEXAPPColl)
                {
                    if (item.XMEUID == IDRegistro.ToString())
                    {
                        result = Convert.ToInt32(item.IDRAMOATIVIDADE);
                        break;
                    }
                }
