...
{
    public partial class TRANSPORTADORAIMEXAPPProvider
    {
        ...
        public async void Save(TRANSPORTADORAIMEXAPPEntity Entity)
        {
            try
            {
                ...
                string URI = BmsSoftware.Modulos.IMEXApp.UrlIMEXApp.Default.PostTransportadora;

                Entity.IDTRANSPORTADORA = null;
                Entity.IDEMPRESA = Convert.ToInt32(CONFISISTEMATy.IDEMPRESAIMEXAPP);
                Entity.IDINTEGRACAO = 0;
                Entity.STATIVO = true;
        ...
        public int GetID(int CodRegistro)
        {
            ...
            try
            {
                ...
                CONFISISTEMATy = CONFISISTEMAP.Read(1);
                string token = CONFISISTEMATy.TOKENIMEXAPP.Trim();
                string URI = BmsSoftware.Modulos.IMEXApp.UrlIMEXApp.Default.GetRegistrosTransportadora;
                ...
                using (HttpClient client = new HttpClient())
                {
                    ...
                    TRANSPORTADORAIMEXAPPColl = DeserializeToList<TRANSPORTADORAIMEXAPPEntity>(jsonString);
                }
        ...
        public int GetID()
        {
            ...
            try
            {
                ...
                if(TRANSPORTADORAIMEXAPPColl.Count > 0)
                    Result = Convert.ToInt32(TRANSPORTADORAIMEXAPPColl[0].IDTRANSPORTADORA);
        ...
        public async void Delete(int CodRegistro)
        {
            try
            {
                ...
                CONFISISTEMATy = CONFISISTEMAP.Read(1);
                string token = CONFISISTEMATy.TOKENIMEXAPP.Trim();
                string URI = BmsSoftware.Modulos.IMEXApp.UrlIMEXApp.Default.DeleteRegistrosTransportadora + token + "/";
        ...
        public int BuscaID(int IDRegistro)
        {
            ...
            try
            {
                foreach (var item in TRANSPORTADORAIMEXAPPColl)
                {
                    if (item.XMEUID == IDRegistro.ToString())
                    {
                        result = Convert.ToInt32(item.IDTRANSPORTADORA);
                        break;
                    }
                }
