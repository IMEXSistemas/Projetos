...
{
    public partial class PRECOIMEXAPPProvider
    {
        CONFISISTEMAProvider CONFISISTEMAP = new CONFISISTEMAProvider();
        CONFISISTEMAEntity CONFISISTEMATy = new CONFISISTEMAEntity();

        IList<PRECOIMEXAPPEntity> PRECOIMEXAPPColl;

        Utility Util = new Utility();

        public async void Save(PRECOIMEXAPPEntity Entity)
        {
            try
            {
        ...
        public int GetID(int CodRegistro)
        {
            ...
            try
            {
                ...
                CONFISISTEMATy = CONFISISTEMAP.Read(1);
                string token = CONFISISTEMATy.TOKENIMEXAPP.Trim();
                string URI = BmsSoftware.Modulos.IMEXApp.UrlIMEXApp.Default.GetRegistrosPreco;
                ...
                using (HttpClient client = new HttpClient())
                {
                    ...
                    PRECOIMEXAPPColl = DeserializeToList<PRECOIMEXAPPEntity>(jsonString);
                }
        ...
        public int GetID()
        {
            ...
            try
            {
                ...
                //Localiza o ID
                if(PRECOIMEXAPPColl.Count > 0)
                    Result = Convert.ToInt32(PRECOIMEXAPPColl[0].IDTABELAPRECO);
        ...
        public int BuscaID(int IDRegistro)
        {
            ...
            try
            {
                foreach (var item in PRECOIMEXAPPColl)
                {
                    if (item.XMEUID == IDRegistro.ToString())
                    {
                        result = Convert.ToInt32(item.IDTABELAPRECO);
                        break;
                    }
                }
