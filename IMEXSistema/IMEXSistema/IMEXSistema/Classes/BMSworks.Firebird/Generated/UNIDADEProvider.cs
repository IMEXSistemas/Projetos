...
{
	public partial class UNIDADEProvider
	{
		...
		public  int Save(UNIDADEEntity Entity )
		{	
			int result = 0;
			...
			try
			{
				...
				if (dbTransaction != null)
				{
					...
					dbCommand = new FbCommand("Sav_UNIDADE", dbCnn);
...
if(Entity.IDUNIDADE!= -1)
	dbCommand.Parameters.AddWithValue("@IDUNIDADE",Entity.IDUNIDADE); //PrimaryKey 
else
	dbCommand.Parameters.AddWithValue("@IDUNIDADE", DBNull.Value); //PrimaryKey 
...
dbCommand.Parameters.AddWithValue("@OBSERVACAO", Entity.OBSERVACAO); //Coluna 
				...
				dbCommand.Parameters["@IDUNIDADE"].Direction = ParameterDirection.InputOutput;

				
				//Executando consulta
				dbCommand.ExecuteNonQuery();
							
			    result = int.Parse(dbCommand.Parameters["@IDUNIDADE"].Value.ToString());
		...
		public  int Save(int? IDUNIDADE, string NOME, string OBSERVACAO)
		{	
			int result = 0;
...
if(IDUNIDADE!= null)
	dbCommand.Parameters.AddWithValue("@IDUNIDADE", IDUNIDADE); //PrimaryKey 
else
	dbCommand.Parameters.AddWithValue("@IDUNIDADE", DBNull.Value); //PrimaryKey 
...
dbCommand.Parameters.AddWithValue("@OBSERVACAO", OBSERVACAO); //Coluna 
				...
				dbCommand.Parameters["@IDUNIDADE"].Direction = ParameterDirection.ReturnValue;
				
				//Executando consulta
				dbCommand.ExecuteNonQuery();
							
			    result = (int)(dbCommand.Parameters["@IDUNIDADE"].Value);
		...
		public  int Delete(int IDUNIDADE)
		{
			int result = 0;
			...
			try
			{
				...
				if (dbTransaction != null)
				{
					...
					dbCommand = new FbCommand("Del_UNIDADE", dbCnn);
					dbCommand.Transaction = ((FbTransaction)(dbTransaction));
				}
				else
				{
		...
		public  UNIDADEEntity Read(int IDUNIDADE)
		{
			...
			try
			{
				...
				if (dbTransaction != null)
				{
					...
					dbCommand = new FbCommand("Rea_UNIDADE", dbCnn);
					dbCommand.Transaction = ((FbTransaction)(dbTransaction));
				}
				else
				{
		...
		public  UNIDADECollection ReadCollectionByParameter(List<RowsFiltro> RowsFiltro)
		{
			...
			try
			{
				if (RowsFiltro != null)
				{
					...
					{
						strSqlCommand = "SELECT * FROM UNIDADE  ";
					}
				}
				else
				{
		...
		private static UNIDADEEntity FillEntityObject(ref FbDataReader DataReader) 
		{
