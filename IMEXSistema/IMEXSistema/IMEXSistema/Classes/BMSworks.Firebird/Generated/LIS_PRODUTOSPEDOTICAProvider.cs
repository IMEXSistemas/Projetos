...
{
	public partial class LIS_PRODUTOSPEDOTICAProvider 
	{
		...
		public  LIS_PRODUTOSPEDOTICACollection ReadCollection()
		{
			FbDataReader dataReader = null;

			try
			{
				LIS_PRODUTOSPEDOTICACollection collection = null;
				...
				if (dbTransaction != null)
				{
					...
					dbCommand = new FbCommand("SELECT * FROM LIS_PRODUTOSPEDOTICA", dbCnn);
		...
		public  LIS_PRODUTOSPEDOTICACollection ReadCollectionByParameter(List<RowsFiltro> RowsFiltro)
		{
			FbDataReader dataReader = null;
			...
			try
			{
				if (RowsFiltro != null)
				{
					if (RowsFiltro.Count > 0)
					{
						...
						string strSqlCommand = "SELECT * FROM LIS_PRODUTOSPEDOTICA WHERE (";
...
public  LIS_PRODUTOSPEDOTICACollection ReadCollectionByParameter(List<RowsFiltro> RowsFiltro,  string FieldOrder)
		{
			FbDataReader dataReader = null;
			...
			try
			{
				if (RowsFiltro != null)
				{
					if (RowsFiltro.Count > 0)
					{
						...
						strSqlCommand = "SELECT * FROM LIS_PRODUTOSPEDOTICA WHERE (";
					...
					{
						strSqlCommand = "SELECT * FROM LIS_PRODUTOSPEDOTICA  order by  " + FieldOrder;
					}
				}
				else
				{
		...
		private static LIS_PRODUTOSPEDOTICACollection ExecuteReader(ref LIS_PRODUTOSPEDOTICACollection collection, ref FbDataReader dataReader, FbCommand dbCommand)
		{
			using (dataReader = dbCommand.ExecuteReader())
			{
				collection = new LIS_PRODUTOSPEDOTICACollection();
		...
		private static LIS_PRODUTOSPEDOTICAEntity FillEntityObject(ref FbDataReader DataReader) 
		{
			LIS_PRODUTOSPEDOTICAEntity entity = new LIS_PRODUTOSPEDOTICAEntity();
			...
			entity.PORCDECONTO = getData.ConvertDBValueToDecimalNullable(DataReader, DataReader.GetOrdinal("PORCDECONTO"));
			entity.VLUNITLIQUIDO = getData.ConvertDBValueToDecimalNullable(DataReader, DataReader.GetOrdinal("VLUNITLIQUIDO"));
