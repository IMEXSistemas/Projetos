...
{
	public partial class TIPOREGIAOProvider
	{
		...
		public  int Save(TIPOREGIAOEntity Entity )
		{	
			int result = 0;
			...
			try
			{
				...
				if (dbTransaction != null)
				{
					...
					dbCommand = new FbCommand("Sav_TIPOREGIAO", dbCnn);
...
if(Entity.IDTIPOREGIAO!= -1)
	dbCommand.Parameters.AddWithValue("@IDTIPOREGIAO",Entity.IDTIPOREGIAO); //PrimaryKey 
else
	dbCommand.Parameters.AddWithValue("@IDTIPOREGIAO", DBNull.Value); //PrimaryKey 
...
dbCommand.Parameters.AddWithValue("@OBSERVACAO", Entity.OBSERVACAO); //Coluna 
				...
				dbCommand.Parameters["@IDTIPOREGIAO"].Direction = ParameterDirection.InputOutput;

				
				//Executando consulta
				dbCommand.ExecuteNonQuery();
							
			    result = int.Parse(dbCommand.Parameters["@IDTIPOREGIAO"].Value.ToString());
		...
		public  int Save(int? IDTIPOREGIAO, string NOME, string OBSERVACAO)
		{	
			int result = 0;
...
dbCommand.Parameters.AddWithValue("@OBSERVACAO", OBSERVACAO); //Coluna 
				...
				dbCommand.Parameters["@IDTIPOREGIAO"].Direction = ParameterDirection.ReturnValue;
				
				//Executando consulta
				dbCommand.ExecuteNonQuery();
							
			    result = (int)(dbCommand.Parameters["@IDTIPOREGIAO"].Value);
		...
		public  int Delete(int IDTIPOREGIAO)
		{
			int result = 0;
			...
			try
			{
				...
				if (dbTransaction != null)
				{
					...
					dbCommand = new FbCommand("Del_TIPOREGIAO", dbCnn);
					dbCommand.Transaction = ((FbTransaction)(dbTransaction));
				}
				else
				{
		...
		public  TIPOREGIAOCollection ReadCollectionByParameter(List<RowsFiltro> RowsFiltro)
		{
			...
			try
			{
				if (RowsFiltro != null)
				{
					...
					{
						strSqlCommand = "SELECT * FROM TIPOREGIAO  ";
					}
				}
				else
				{
		...
		private static TIPOREGIAOEntity FillEntityObject(ref FbDataReader DataReader) 
		{
			TIPOREGIAOEntity entity = new TIPOREGIAOEntity();
