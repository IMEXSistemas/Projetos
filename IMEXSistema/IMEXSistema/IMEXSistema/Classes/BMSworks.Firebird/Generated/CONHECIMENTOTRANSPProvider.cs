...
{
	public partial class CONHECIMENTOTRANSPProvider
	{
		...
		public  int Save(int? IDCONHECIMENTOTRANSP, DateTime DATA, string NDOCUMENTO, string MODELO, string SERIE, int IDTRANSPORTADORA, int IDCFOP, decimal VLTOTAL, decimal VLBASEICMS, decimal VLICMS, decimal OUTRAS, string OBSERVACAO, string MODALIDADE)
		{	
			int result = 0;
			...
			try
			{
				...
				dbCommand.CommandType = CommandType.StoredProcedure;
									...
									//PrimaryKey com valor igual a null, indica um novo registro, 
									//o valor da chave será fornecido pelo banco. Qualquer outro valor indicará edição do registro.
									if (IDCONHECIMENTOTRANSP == -1)
										dbCommand.Parameters.AddWithValue("@IDCONHECIMENTOTRANSP", DBNull.Value);
									else
										dbCommand.Parameters.AddWithValue("@IDCONHECIMENTOTRANSP", IDCONHECIMENTOTRANSP);
										...
										if(IDTRANSPORTADORA!= null)
											dbCommand.Parameters.AddWithValue("@IDTRANSPORTADORA", IDTRANSPORTADORA); //ForeignKey 
										else
											dbCommand.Parameters.AddWithValue("@IDTRANSPORTADORA", DBNull.Value); //ForeignKey 5
										...
										dbCommand.Parameters.AddWithValue("@VLBASEICMS", VLBASEICMS); //Coluna 
										...
										dbCommand.Parameters.AddWithValue("@MODALIDADE", MODALIDADE); //Coluna 
		...
		public  int Delete(int IDCONHECIMENTOTRANSP)
		{
			int result = 0;
			...
			try
			{
				...
				if (dbTransaction != null)
				{
					...
					dbCommand = new FbCommand("Del_CONHECIMENTOTRANSP", dbCnn);
					dbCommand.Transaction = ((FbTransaction)(dbTransaction));
				}
				else
				{
				...
				dbCommand.Parameters.AddWithValue("@IDCONHECIMENTOTRANSP",IDCONHECIMENTOTRANSP); //PrimaryKey
		...
		public  CONHECIMENTOTRANSPEntity Read(int IDCONHECIMENTOTRANSP)
		{
			...
			try
			{
				...
				if (dbTransaction != null)
				{
					...
					dbCommand = new FbCommand("Rea_CONHECIMENTOTRANSP", dbCnn);
					dbCommand.Transaction = ((FbTransaction)(dbTransaction));
				}
				else
				{
				...
				CONHECIMENTOTRANSPEntity entity = null;
				if (reader.HasRows)
				{
					while (reader.Read())
					{
		...
		public  CONHECIMENTOTRANSPCollection ReadCollectionByParameter(List<RowsFiltro> RowsFiltro)
		{
			FbDataReader dataReader = null;
			CONHECIMENTOTRANSPCollection collection = null;
			...
			try
			{
				if (RowsFiltro != null)
				{
					...
					{
						strSqlCommand = "SELECT * FROM CONHECIMENTOTRANSP WHERE (";
					...
					{
						strSqlCommand = "SELECT * FROM CONHECIMENTOTRANSP  ";
					}
				}
				else
				{
		...
		public  CONHECIMENTOTRANSPCollection ReadCollectionByParameter(List<RowsFiltro> RowsFiltro, string FieldOrder)
		{
			FbDataReader dataReader = null;
			CONHECIMENTOTRANSPCollection collection = null;
			...
			try
			{
				if (RowsFiltro != null)
				{
					...
					{
						strSqlCommand = "SELECT * FROM CONHECIMENTOTRANSP  order by  " + FieldOrder;
					}
				}
				else
				{
		...
		private static CONHECIMENTOTRANSPCollection ExecuteReader(ref CONHECIMENTOTRANSPCollection collection, ref FbDataReader dataReader, FbCommand dbCommand)
		{
			using (dataReader = dbCommand.ExecuteReader())
			{
				collection = new CONHECIMENTOTRANSPCollection();
		...
		private static CONHECIMENTOTRANSPEntity FillEntityObject(ref FbDataReader DataReader) 
		{
			CONHECIMENTOTRANSPEntity entity = new CONHECIMENTOTRANSPEntity();
