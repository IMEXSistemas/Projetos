...
{
	public partial class SEGURADORAProvider
	{
		...
		public  int Save(SEGURADORAEntity Entity )
		{	
			int result = 0;
			...
			try
			{
				...
				if (dbTransaction != null)
				{
					...
					dbCommand = new FbCommand("Sav_SEGURADORA", dbCnn);
				...
				if (Entity.IDSEGURADORA == -1)
					dbCommand.Parameters.AddWithValue("@IDSEGURADORA", DBNull.Value);
				else
					dbCommand.Parameters.AddWithValue("@IDSEGURADORA", Entity.IDSEGURADORA);
					...
					dbCommand.Parameters.AddWithValue("@EMAILSEGURADORA", Entity.EMAILSEGURADORA); //Coluna 
				...
				dbCommand.Parameters["@IDSEGURADORA"].Direction = ParameterDirection.InputOutput;

				
				//Executando consulta
				dbCommand.ExecuteNonQuery();
							
			    result = int.Parse(dbCommand.Parameters["@IDSEGURADORA"].Value.ToString());
		...
		public  int Save(int? IDSEGURADORA, string NOME, string TELEFONE1, string TELEFONE2, string CNPJ, string IE, string ENDERECO, string BAIRRO, string CIDADE, string UF, string CEP, string EMAILSEGURADORA, string OBSERVACAO, string NUMERO)
		{	
			int result = 0;
			...
			try
			{
				...
				dbCommand.CommandType = CommandType.StoredProcedure;
									...
									if (IDSEGURADORA == -1)
										...
										dbCommand.Parameters.AddWithValue("@IDSEGURADORA", IDSEGURADORA);
										...
										dbCommand.Parameters.AddWithValue("@EMAILSEGURADORA", EMAILSEGURADORA); //Coluna 
		...
		public  int Delete(int IDSEGURADORA)
		{
			int result = 0;
			...
			try
			{
				...
				if (dbTransaction != null)
				{
					...
					dbCommand = new FbCommand("Del_SEGURADORA", dbCnn);
					dbCommand.Transaction = ((FbTransaction)(dbTransaction));
				}
				else
				{
				...
				dbCommand.Parameters.AddWithValue("@IDSEGURADORA",IDSEGURADORA); //PrimaryKey
		...
		public  SEGURADORAEntity Read(int IDSEGURADORA)
		{
			...
			try
			{
				...
				if (dbTransaction != null)
				{
					...
					dbCommand = new FbCommand("Rea_SEGURADORA", dbCnn);
					dbCommand.Transaction = ((FbTransaction)(dbTransaction));
				}
				else
				{
		...
		public  SEGURADORACollection ReadCollectionByParameter(List<RowsFiltro> RowsFiltro)
		{
			FbDataReader dataReader = null;
			SEGURADORACollection collection = null;
			...
			try
			{
				if (RowsFiltro != null)
				{
					...
					{
						strSqlCommand = "SELECT * FROM SEGURADORA WHERE (";
					...
					{
						strSqlCommand = "SELECT * FROM SEGURADORA  ";
					}
				}
				else
				{
		...
		public  SEGURADORACollection ReadCollectionByParameter(List<RowsFiltro> RowsFiltro, string FieldOrder)
		{
			FbDataReader dataReader = null;
			SEGURADORACollection collection = null;
			...
			try
			{
				if (RowsFiltro != null)
				{
					...
					{
						strSqlCommand = "SELECT * FROM SEGURADORA  order by  " + FieldOrder;
					}
				}
				else
				{
		...
		private static SEGURADORACollection ExecuteReader(ref SEGURADORACollection collection, ref FbDataReader dataReader, FbCommand dbCommand)
		{
			using (dataReader = dbCommand.ExecuteReader())
			{
				collection = new SEGURADORACollection();
		...
		private static SEGURADORAEntity FillEntityObject(ref FbDataReader DataReader) 
		{
			SEGURADORAEntity entity = new SEGURADORAEntity();

			FirebirdGetDbData getData = new FirebirdGetDbData();

							entity.IDSEGURADORA = getData.ConvertDBValueToInt32(DataReader, DataReader.GetOrdinal("IDSEGURADORA"));
			...
			entity.EMAILSEGURADORA = getData.ConvertDBValueToStringNullable(DataReader, DataReader.GetOrdinal("EMAILSEGURADORA"));
