...
{
	public partial class LIS_ORDEMSERVICOProvider 
	{
		...
		public  LIS_ORDEMSERVICOCollection ReadCollection()
		{
			FbDataReader dataReader = null;

			try
			{
				LIS_ORDEMSERVICOCollection collection = null;
				...
				if (dbTransaction != null)
				{
					...
					dbCommand = new FbCommand("SELECT * FROM LIS_ORDEMSERVICO", dbCnn);
		...
		public  LIS_ORDEMSERVICOCollection ReadCollectionByParameter(List<RowsFiltro> RowsFiltro)
		{
			FbDataReader dataReader = null;
			...
			try
			{
				if (RowsFiltro != null)
				{
					if (RowsFiltro.Count > 0)
					{
						...
						string strSqlCommand = "SELECT * FROM LIS_ORDEMSERVICO WHERE (";
...
public  LIS_ORDEMSERVICOCollection ReadCollectionByParameter(List<RowsFiltro> RowsFiltro,  string FieldOrder)
		{
			FbDataReader dataReader = null;
			...
			try
			{
				if (RowsFiltro != null)
				{
					if (RowsFiltro.Count > 0)
					{
						...
						strSqlCommand = "SELECT * FROM LIS_ORDEMSERVICO WHERE (";
					...
					{
						strSqlCommand = "SELECT * FROM LIS_ORDEMSERVICO  order by  " + FieldOrder;
					}
				}
				else
				{
		...
		private static LIS_ORDEMSERVICOCollection ExecuteReader(ref LIS_ORDEMSERVICOCollection collection, ref FbDataReader dataReader, FbCommand dbCommand)
		{
			using (dataReader = dbCommand.ExecuteReader())
			{
				collection = new LIS_ORDEMSERVICOCollection();
		...
		private static LIS_ORDEMSERVICOEntity FillEntityObject(ref FbDataReader DataReader) 
		{
			LIS_ORDEMSERVICOEntity entity = new LIS_ORDEMSERVICOEntity();
			...
			entity.NOMEATENDIMENTO = getData.ConvertDBValueToStringNullable(DataReader, DataReader.GetOrdinal("NOMEATENDIMENTO"));
