...
{
	public partial class CAIXAProvider
	{
...
dbCommand.Parameters.AddWithValue("@DATAMOV", Entity.DATAMOV); //Coluna 
		...
		public  int Save(int? IDCAIXA, int IDTIPOMOVCAIXA, string NDOCUMENTO, int IDTIPODUPLICATA, int IDCENTROCUSTOS, decimal VALOR, string OBSERVACAO, DateTime DATAMOV)
		{	
			int result = 0;
...
dbCommand.Parameters.AddWithValue("@DATAMOV", DATAMOV); //Coluna 
		...
		public  int Delete(int IDCAIXA)
		{
			...
			try
			{
				...
				if (dbTransaction != null)
				{
					...
					dbCommand = new FbCommand("Del_CAIXA", dbCnn);
					dbCommand.Transaction = ((FbTransaction)(dbTransaction));
				}
				else
				{
		...
		public  CAIXACollection ReadCollectionByParameter(List<RowsFiltro> RowsFiltro)
		{
			FbDataReader dataReader = null;
			CAIXACollection collection = null;
			...
			try
			{
				if (RowsFiltro != null)
				{
					...
					{
						strSqlCommand = "SELECT * FROM CAIXA WHERE (";
					...
					{
						strSqlCommand = "SELECT * FROM CAIXA  ";
					}
				}
				else
				{
		...
		public  CAIXACollection ReadCollectionByParameter(List<RowsFiltro> RowsFiltro, string FieldOrder)
		{
			FbDataReader dataReader = null;
			CAIXACollection collection = null;
			...
			try
			{
				if (RowsFiltro != null)
				{
					...
					{
						strSqlCommand = "SELECT * FROM CAIXA  order by  " + FieldOrder;
					}
				}
				else
				{
		...
		private static CAIXACollection ExecuteReader(ref CAIXACollection collection, ref FbDataReader dataReader, FbCommand dbCommand)
		{
			using (dataReader = dbCommand.ExecuteReader())
			{
				collection = new CAIXACollection();
