...
{
	public partial class ORCAMENTOProvider
	{
		...
		public  int Save(ORCAMENTOEntity Entity )
		{	
			int result = 0;
			...
			try
			{
				...
				if (dbTransaction != null)
				{
					...
					dbCommand = new FbCommand("Sav_ORCAMENTO", dbCnn);
...
if(Entity.IDORCAMENTO!= -1)
	dbCommand.Parameters.AddWithValue("@IDORCAMENTO",Entity.IDORCAMENTO); //PrimaryKey 
else
	dbCommand.Parameters.AddWithValue("@IDORCAMENTO", DBNull.Value); //PrimaryKey 
...
dbCommand.Parameters.AddWithValue("@TOTAORCAMENTO", Entity.TOTAORCAMENTO); //Coluna 
				...
				dbCommand.Parameters["@IDORCAMENTO"].Direction = ParameterDirection.InputOutput;

				
				//Executando consulta
				dbCommand.ExecuteNonQuery();
							
			    result = int.Parse(dbCommand.Parameters["@IDORCAMENTO"].Value.ToString());
...
if(IDORCAMENTO!= -1)
	dbCommand.Parameters.AddWithValue("@IDORCAMENTO", IDORCAMENTO); //PrimaryKey 
else
	dbCommand.Parameters.AddWithValue("@IDORCAMENTO", DBNull.Value); //PrimaryKey 

dbCommand.Parameters.AddWithValue("@IDCLIENTE", IDCLIENTE); //Coluna 
dbCommand.Parameters.AddWithValue("@DTEMISSAO", DTEMISSAO); //Coluna 
dbCommand.Parameters.AddWithValue("@DTVALIDADE", DTVALIDADE); //Coluna 
...
dbCommand.Parameters.AddWithValue("@TOTALDESCONTO", TOTALDESCONTO); //Coluna 
dbCommand.Parameters.AddWithValue("@PORCACRESCIMO", PORCACRESCIMO); //Coluna 
dbCommand.Parameters.AddWithValue("@TOTALACRESCIMO", TOTALACRESCIMO); //Coluna 
dbCommand.Parameters.AddWithValue("@TOTAORCAMENTO", TOTAORCAMENTO); //Coluna 
		...
		public  int Delete(int IDORCAMENTO)
		{
			int result = 0;
			...
			try
			{
				...
				if (dbTransaction != null)
				{
					...
					dbCommand = new FbCommand("Del_ORCAMENTO", dbCnn);
					dbCommand.Transaction = ((FbTransaction)(dbTransaction));
				}
				else
				{
		...
		public  ORCAMENTOEntity Read(int IDORCAMENTO)
		{
			...
			try
			{
				...
				if (dbTransaction != null)
				{
					...
					dbCommand = new FbCommand("Rea_ORCAMENTO", dbCnn);
					dbCommand.Transaction = ((FbTransaction)(dbTransaction));
				}
				else
				{
		...
		public  ORCAMENTOCollection ReadCollectionByParameter(List<RowsFiltro> RowsFiltro)
		{
			FbDataReader dataReader = null;
			ORCAMENTOCollection collection = null;
			...
			try
			{
				if (RowsFiltro != null)
				{
					...
					{
						strSqlCommand = "SELECT * FROM ORCAMENTO WHERE (";
					...
					{
						strSqlCommand = "SELECT * FROM ORCAMENTO  ";
					}
				}
				else
				{
		...
		public  ORCAMENTOCollection ReadCollectionByParameter(List<RowsFiltro> RowsFiltro, string FieldOrder)
		{
			FbDataReader dataReader = null;
			ORCAMENTOCollection collection = null;
			...
			try
			{
				if (RowsFiltro != null)
				{
					...
					{
						strSqlCommand = "SELECT * FROM ORCAMENTO  order by  " + FieldOrder;
					}
				}
				else
				{
		...
		private static ORCAMENTOCollection ExecuteReader(ref ORCAMENTOCollection collection, ref FbDataReader dataReader, FbCommand dbCommand)
		{
			using (dataReader = dbCommand.ExecuteReader())
			{
				collection = new ORCAMENTOCollection();
		...
		private static ORCAMENTOEntity FillEntityObject(ref FbDataReader DataReader) 
		{
			ORCAMENTOEntity entity = new ORCAMENTOEntity();

			FirebirdGetDbData getData = new FirebirdGetDbData();

							entity.IDORCAMENTO = getData.ConvertDBValueToInt32(DataReader, DataReader.GetOrdinal("IDORCAMENTO"));
			...
			entity.DTVALIDADE = getData.ConvertDBValueToDateTimeNullable(DataReader, DataReader.GetOrdinal("DTVALIDADE"));
			...
			entity.TOTALDESCONTO = getData.ConvertDBValueToDecimalNullable(DataReader, DataReader.GetOrdinal("TOTALDESCONTO"));
			...
			entity.TOTAORCAMENTO = getData.ConvertDBValueToDecimalNullable(DataReader, DataReader.GetOrdinal("TOTAORCAMENTO"));
