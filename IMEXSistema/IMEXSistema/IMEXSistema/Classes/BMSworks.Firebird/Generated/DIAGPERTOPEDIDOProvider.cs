...
{
	public partial class DIAGPERTOPEDIDOProvider
	{
		...
		public  int Save(DIAGPERTOPEDIDOEntity Entity )
		{	
			int result = 0;
			...
			try
			{
				...
				if (dbTransaction != null)
				{
					...
					dbCommand = new FbCommand("Sav_DIAGPERTOPEDIDO", dbCnn);
			...
			if(Entity.IDDIAGPERTOPEDIDO!= -1)
				dbCommand.Parameters.AddWithValue("@IDDIAGPERTOPEDIDO",Entity.IDDIAGPERTOPEDIDO); //PrimaryKey 
			else
				dbCommand.Parameters.AddWithValue("@IDDIAGPERTOPEDIDO", DBNull.Value); //PrimaryKey 
			...
			dbCommand.Parameters.AddWithValue("@MV", Entity.MV); //Coluna 
				...
				dbCommand.Parameters["@IDDIAGPERTOPEDIDO"].Direction = ParameterDirection.InputOutput;

				
				//Executando consulta
				dbCommand.ExecuteNonQuery();
							
			    result = int.Parse(dbCommand.Parameters["@IDDIAGPERTOPEDIDO"].Value.ToString());
		...
		{	
			...
			if(IDDIAGPERTOPEDIDO!= -1)
				dbCommand.Parameters.AddWithValue("@IDDIAGPERTOPEDIDO", IDDIAGPERTOPEDIDO); //PrimaryKey 
			else
				dbCommand.Parameters.AddWithValue("@IDDIAGPERTOPEDIDO", DBNull.Value); //PrimaryKey 
		...
		public  int Delete(int IDDIAGPERTOPEDIDO)
		{
			int result = 0;
			...
			try
			{
				...
				if (dbTransaction != null)
				{
					...
					dbCommand = new FbCommand("Del_DIAGPERTOPEDIDO", dbCnn);
					dbCommand.Transaction = ((FbTransaction)(dbTransaction));
				}
				else
				{
		...
		public  DIAGPERTOPEDIDOEntity Read(int IDDIAGPERTOPEDIDO)
		{
			...
			try
			{
				...
				if (dbTransaction != null)
				{
					...
					dbCommand = new FbCommand("Rea_DIAGPERTOPEDIDO", dbCnn);
					dbCommand.Transaction = ((FbTransaction)(dbTransaction));
				}
				else
				{
				...
				DIAGPERTOPEDIDOEntity entity = null;
				if (reader.HasRows)
				{
					while (reader.Read())
					{
		...
		public  DIAGPERTOPEDIDOCollection ReadCollectionByParameter(List<RowsFiltro> RowsFiltro)
		{
			FbDataReader dataReader = null;
			DIAGPERTOPEDIDOCollection collection = null;
			...
			try
			{
				if (RowsFiltro != null)
				{
					...
					{
						strSqlCommand = "SELECT * FROM DIAGPERTOPEDIDO WHERE (";
					...
					{
						strSqlCommand = "SELECT * FROM DIAGPERTOPEDIDO  ";
					}
				}
				else
				{
		...
		public  DIAGPERTOPEDIDOCollection ReadCollectionByParameter(List<RowsFiltro> RowsFiltro, string FieldOrder)
		{
			FbDataReader dataReader = null;
			DIAGPERTOPEDIDOCollection collection = null;
			...
			try
			{
				if (RowsFiltro != null)
				{
					...
					{
						strSqlCommand = "SELECT * FROM DIAGPERTOPEDIDO  order by  " + FieldOrder;
					}
				}
				else
				{
		...
		private static DIAGPERTOPEDIDOCollection ExecuteReader(ref DIAGPERTOPEDIDOCollection collection, ref FbDataReader dataReader, FbCommand dbCommand)
		{
			using (dataReader = dbCommand.ExecuteReader())
			{
				collection = new DIAGPERTOPEDIDOCollection();
		...
		private static DIAGPERTOPEDIDOEntity FillEntityObject(ref FbDataReader DataReader) 
		{
			DIAGPERTOPEDIDOEntity entity = new DIAGPERTOPEDIDOEntity();

			FirebirdGetDbData getData = new FirebirdGetDbData();

							entity.IDDIAGPERTOPEDIDO = getData.ConvertDBValueToInt32(DataReader, DataReader.GetOrdinal("IDDIAGPERTOPEDIDO"));
