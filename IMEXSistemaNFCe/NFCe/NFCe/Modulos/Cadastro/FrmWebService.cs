...
namespace BmsSoftware.Modulos.Cadastros
{
    public partial class FrmWebService : Form
    {
        ...
        public int _WEBSERVICEID = -1;
        public WEBSERVICEEntity Entity
        {
            get
            {
                  string  CAMINHO = txtCaminho.Text;      //VARCHAR(100),
                  int  IDUF  = Convert.ToInt32(cbUF2.SelectedValue);//         INTEGER,
                  string  OBSERVACAO  = txtObservacao.Text;

              return new WEBSERVICEEntity(_WEBSERVICEID, CAMINHO, IDUF, OBSERVACAO);
            }
            set
            {
                ...
                if (value != null)
                {
                    _WEBSERVICEID = value.WEBSERVICEID;
                    txtCaminho.Text = value.CAMINHO;
                    cbUF2.SelectedValue = value.IDUF;
        ...
        private void Grava()
        {
            try
            {
                if (Validacoes())
                {
                    _WEBSERVICEID = WEBSERVICEP.Save(Entity);
                    GetAllLocalCobranca();
        ...
        private void GetAllLocalCobranca()
        {
            WEBSERVICEColl = WEBSERVICEP.ReadCollectionByParameter(null, "WEBSERVICEID");
            DataGriewDados.AutoGenerateColumns = false;
            DataGriewDados.DataSource = WEBSERVICEColl;

            lblTotalPesquisa.Text = WEBSERVICEColl.Count.ToString();
        }
        ...
        private Boolean Validacoes()
        {
            ...
            {
                errorProvider1.SetError(txtCaminho, ConfigMessage.Default.CampoObrigatorio);
        ...
        private void GetDropUF2()
        {
            try
            {
                ...
                cbUF2.ValueMember = "CODIGO_UF_IBGE";
                ...
                cbUF2.SelectedIndex = cbUF2.FindString(EMPRESAP.Read(1).UF);
            }
            catch (Exception ex)
            {
        ...
        private void Delete()
        {
            ...
            else
            {
                ...
                if (dr == DialogResult.Yes)
                {
                    try
                    {
                        WEBSERVICEP.Delete(_WEBSERVICEID);
        ...
        {
            if (WEBSERVICEColl.Count > 0)
            {
                int rowindex = e.RowIndex;
                if (rowindex != -1)
                {
                    int CodigoSelect = Convert.ToInt32(WEBSERVICEColl[rowindex].WEBSERVICEID);

                    Entity = WEBSERVICEP.Read(CodigoSelect);
        ...
        {
            RelatorioTitulo = InputBox("Título do Relatório", ConfigSistema1.Default.NomeEmpresa, "Lista Local de Cobrança");
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            try
            {
                ...
                int NumerorRegistros = WEBSERVICEColl.Count;
                ...
                while (IndexRegistro < WEBSERVICEColl.Count)
                {
                    ...
                    e.Graphics.DrawString(WEBSERVICEColl[IndexRegistro].WEBSERVICEID.ToString(), config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha);
                    e.Graphics.DrawString(Util.LimiterText(WEBSERVICEColl[IndexRegistro].CAMINHO, 40), config.FonteConteudo, Brushes.Black, config.MargemEsquerda + 50, config.PosicaoDaLinha);
                    e.Graphics.DrawString(Util.LimiterText(WEBSERVICEColl[IndexRegistro].OBSERVACAO, 100), config.FonteConteudo, Brushes.Black, config.MargemEsquerda + 250, config.PosicaoDaLinha);
                ...
                if (IndexRegistro < WEBSERVICEColl.Count)
                    e.HasMorePages = true;
                else
                {
                    e.Graphics.DrawString("", config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha + 50);
                    e.Graphics.DrawString("Total da pesquisa: " + WEBSERVICEColl.Count, config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha + 50);
        ...
        private void DataGriewDados_KeyDown(object sender, KeyEventArgs e)
        {
            if (WEBSERVICEColl.Count > 0)
            {
                ...
                int CodigoSelect = Convert.ToInt32(WEBSERVICEColl[indice].WEBSERVICEID);
                ...
                else if (e.KeyCode == Keys.Delete)
                {
                    ...
                    if (dr == DialogResult.Yes)
                    {
                        try
                        {
                            WEBSERVICEP.Delete(CodigoSelect);
                            MessageBox.Show(ConfigMessage.Default.MsgDelete2);
                            GetAllLocalCobranca();
                        }
                        catch (Exception)
