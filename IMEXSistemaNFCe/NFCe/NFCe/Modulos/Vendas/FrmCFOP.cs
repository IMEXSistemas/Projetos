...
namespace BmsSoftware.Modulos.Vendas
{
    public partial class FrmCFOP : Form
    {
        ...
        public int _IdCFOP = -1;
        public CFOPEntity Entity
        {
            get
            {
                string CodCFOP = txtCodCFOP.Text;
                string Descricao = txtDescricao.Text;
                string FlagBaixaEstoque = chkBaixaEstoque.Checked ? "S" : "N";
                string OBSERVACAO = txtObservacao.Text;
                
                return new CFOPEntity(_IdCFOP, CodCFOP, Descricao, FlagBaixaEstoque, OBSERVACAO);
            }
            set
            {
                ...
                if (value != null)
                {
                    _IdCFOP = value.IDCFOP;
                    txtCodCFOP.Text = value.CODCFOP;
                    txtDescricao.Text = value.DESCRICAO;
                    chkBaixaEstoque.Checked =  value.FLAGBAIXAESTOQUE =="S" ? true: false;
        ...
        private void GetAllCFOP()
        {
            CFOPColl = CFOPP.ReadCollectionByParameter(null, "DESCRICAO");
            DataGriewDados.AutoGenerateColumns = false;
            DataGriewDados.DataSource = CFOPColl;

            lblTotalPesquisa.Text = CFOPColl.Count.ToString();
        }
        ...
        private Boolean Validacoes()
        {
            ...
            errorProvider1.Clear();
            if (txtCodCFOP.Text.Trim().Length == 0)
            {
                errorProvider1.SetError(txtCodCFOP, ConfigMessage.Default.CampoObrigatorio);
                MessageBox.Show(ConfigMessage.Default.CampoObrigatorio2);
                result = false;
            }
            else
                errorProvider1.SetError(txtCodCFOP, "");
        ...
        private void DataGriewDados_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if (CFOPColl.Count > 0)
            {
                ...
                if (rowindex != -1)
                {
                    int CodigoSelect = Convert.ToInt32(CFOPColl[rowindex].IDCFOP);

                    Entity = CFOPP.Read(CodigoSelect);
        ...
        {
            RelatorioTitulo = InputBox("Título do Relatório", ConfigSistema1.Default.NomeEmpresa, "Lista de CFOPs");
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            try
            {
                ...
                e.Graphics.DrawString("CFOP", config.FonteNegrito, Brushes.Black, config.MargemEsquerda, 170);
                e.Graphics.DrawString("Descrição", config.FonteNegrito, Brushes.Black, config.MargemEsquerda + 50, 170);
                e.Graphics.DrawString("Observação", config.FonteNegrito, Brushes.Black, config.MargemEsquerda + 300, 170);
                ...
                int NumerorRegistros = CFOPColl.Count;

                while (IndexRegistro < CFOPColl.Count)
                {
                    config.PosicaoDaLinha = config.MargemSuperior + (config.LinhaAtual * config.FonteNormal.GetHeight(e.Graphics));
                    e.Graphics.DrawString(CFOPColl[IndexRegistro].CODCFOP, config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha);
                    e.Graphics.DrawString(Util.LimiterText(CFOPColl[IndexRegistro].DESCRICAO, 30), config.FonteConteudo, Brushes.Black, config.MargemEsquerda + 50, config.PosicaoDaLinha);
                    e.Graphics.DrawString(Util.LimiterText(CFOPColl[IndexRegistro].OBSERVACAO, 100), config.FonteConteudo, Brushes.Black, config.MargemEsquerda + 300, config.PosicaoDaLinha);
                ...
                if (IndexRegistro < CFOPColl.Count)
                    e.HasMorePages = true;
                else
                {
                    e.Graphics.DrawString("", config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha + 50);
                    e.Graphics.DrawString("Total da pesquisa: " + CFOPColl.Count, config.FonteConteudo, Brushes.Black, config.MargemEsquerda, config.PosicaoDaLinha + 50);
        ...
        private void DataGriewDados_KeyDown(object sender, KeyEventArgs e)
        {
            if (CFOPColl.Count > 0)
            {
                ...
                int CodigoSelect = Convert.ToInt32(CFOPColl[indice].IDCFOP);
