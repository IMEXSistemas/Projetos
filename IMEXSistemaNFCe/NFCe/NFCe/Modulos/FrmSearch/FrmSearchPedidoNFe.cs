    ...
    public partial class FrmSearchPedidoNFe : Form
    {
        ...
        private void DataGriewSearch_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (LIS_PEDIDOColl.Count > 0)
            {
                ...
                if (ColumnSelecionada == 0)//Visualizar Produtos
                {
                    using (FrmVisualizaProdutoPedido frm = new FrmVisualizaProdutoPedido())
                    {
                        frm._IDPEDIDO = Convert.ToInt32(LIS_PEDIDOColl[rowindex].IDPEDIDO);
                        frm.ShowDialog();
                    }
                }
                else
