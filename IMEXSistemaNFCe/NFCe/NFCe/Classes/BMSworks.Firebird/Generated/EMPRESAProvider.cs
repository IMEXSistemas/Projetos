...
{
	public partial class EMPRESAProvider
	{
...
dbCommand.Parameters.AddWithValue("@NOMEFANTASIA", Entity.NOMEFANTASIA); //Coluna 
		...
		public  int Save(int? IDEMPRESA, string NOMECLIENTE, string ENDERECO, string BAIRRO, string CEP, string CIDADE, string UF, string TELEFONE, string FAX, string CNPJCPF, string IE, string EMAIL, string REGISTRO, string NUMERO, string COMPLEMENTO, string NOMEFANTASIA)
		{	
			int result = 0;
...
if(IDEMPRESA!= -1)
	dbCommand.Parameters.AddWithValue("@IDEMPRESA", IDEMPRESA); //PrimaryKey 
else
	dbCommand.Parameters.AddWithValue("@IDEMPRESA", DBNull.Value); //PrimaryKey 

dbCommand.Parameters.AddWithValue("@NOMECLIENTE", NOMECLIENTE); //Coluna 
...
dbCommand.Parameters.AddWithValue("@CNPJCPF", CNPJCPF); //Coluna 
dbCommand.Parameters.AddWithValue("@IE", IE); //Coluna 
dbCommand.Parameters.AddWithValue("@EMAIL", EMAIL); //Coluna 
dbCommand.Parameters.AddWithValue("@REGISTRO", REGISTRO); //Coluna 
...
dbCommand.Parameters.AddWithValue("@NOMEFANTASIA", NOMEFANTASIA); //Coluna 
		...
		public  int Delete(int IDEMPRESA)
		{
			...
			try
			{
				...
				if (dbTransaction != null)
				{
					...
					dbCommand = new FbCommand("Del_EMPRESA", dbCnn);
					dbCommand.Transaction = ((FbTransaction)(dbTransaction));
				}
				else
				{
		...
		public  EMPRESAEntity Read(int IDEMPRESA)
		{
			...
			try
			{
				...
				if (dbTransaction != null)
				{
					...
					dbCommand = new FbCommand("Rea_EMPRESA", dbCnn);
					dbCommand.Transaction = ((FbTransaction)(dbTransaction));
				}
				else
				{
		...
		public  EMPRESACollection ReadCollectionByParameter(List<RowsFiltro> RowsFiltro)
		{
			FbDataReader dataReader = null;
			EMPRESACollection collection = null;
			...
			try
			{
				if (RowsFiltro != null)
				{
					...
					{
						strSqlCommand = "SELECT * FROM EMPRESA WHERE (";
					...
					{
						strSqlCommand = "SELECT * FROM EMPRESA  ";
					}
				}
				else
				{
		...
		public  EMPRESACollection ReadCollectionByParameter(List<RowsFiltro> RowsFiltro, string FieldOrder)
		{
			FbDataReader dataReader = null;
			EMPRESACollection collection = null;
			...
			try
			{
				if (RowsFiltro != null)
				{
					...
					{
						strSqlCommand = "SELECT * FROM EMPRESA  order by  " + FieldOrder;
					}
				}
				else
				{
		...
		private static EMPRESACollection ExecuteReader(ref EMPRESACollection collection, ref FbDataReader dataReader, FbCommand dbCommand)
		{
		...
		private static EMPRESAEntity FillEntityObject(ref FbDataReader DataReader) 
		{
			EMPRESAEntity entity = new EMPRESAEntity();

			FirebirdGetDbData getData = new FirebirdGetDbData();

							entity.IDEMPRESA = getData.ConvertDBValueToInt32(DataReader, DataReader.GetOrdinal("IDEMPRESA"));
			...
			entity.CNPJCPF = getData.ConvertDBValueToStringNullable(DataReader, DataReader.GetOrdinal("CNPJCPF"));
			...
			entity.REGISTRO = getData.ConvertDBValueToStringNullable(DataReader, DataReader.GetOrdinal("REGISTRO"));
