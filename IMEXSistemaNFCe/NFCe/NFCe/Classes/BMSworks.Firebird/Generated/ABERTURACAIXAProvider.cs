...
{
	public partial class ABERTURACAIXAProvider
	{
		...
		public  int Save(int? IDABERTURACAIXA, DateTime DATA, int IDFUNCIONARIO, decimal VALOR)
		{	
			int result = 0;
			...
			try
			{
				...
				dbCommand.CommandType = CommandType.StoredProcedure;
									...
									//PrimaryKey com valor igual a null, indica um novo registro, 
									//o valor da chave será fornecido pelo banco. Qualquer outro valor indicará edição do registro.
									if (IDABERTURACAIXA == -1)
										dbCommand.Parameters.AddWithValue("@IDABERTURACAIXA", DBNull.Value);
									else
										dbCommand.Parameters.AddWithValue("@IDABERTURACAIXA", IDABERTURACAIXA);
		...
		public  int Delete(int IDABERTURACAIXA)
		{
			int result = 0;
			...
			try
			{
				...
				if (dbTransaction != null)
				{
					...
					dbCommand = new FbCommand("Del_ABERTURACAIXA", dbCnn);
					dbCommand.Transaction = ((FbTransaction)(dbTransaction));
				}
				else
				{
				...
				dbCommand.Parameters.AddWithValue("@IDABERTURACAIXA",IDABERTURACAIXA); //PrimaryKey
		...
		public  ABERTURACAIXAEntity Read(int IDABERTURACAIXA)
		{
			...
			try
			{
				...
				if (dbTransaction != null)
				{
					...
					dbCommand = new FbCommand("Rea_ABERTURACAIXA", dbCnn);
					dbCommand.Transaction = ((FbTransaction)(dbTransaction));
				}
				else
				{
				...
				ABERTURACAIXAEntity entity = null;
				if (reader.HasRows)
				{
					while (reader.Read())
					{
		...
		public  ABERTURACAIXACollection ReadCollectionByParameter(List<RowsFiltro> RowsFiltro)
		{
			FbDataReader dataReader = null;
			ABERTURACAIXACollection collection = null;
			...
			try
			{
				if (RowsFiltro != null)
				{
					...
					{
						strSqlCommand = "SELECT * FROM ABERTURACAIXA WHERE (";
					...
					{
						strSqlCommand = "SELECT * FROM ABERTURACAIXA  ";
					}
				}
				else
				{
		...
		public  ABERTURACAIXACollection ReadCollectionByParameter(List<RowsFiltro> RowsFiltro, string FieldOrder)
		{
			FbDataReader dataReader = null;
			ABERTURACAIXACollection collection = null;
			...
			try
			{
				if (RowsFiltro != null)
				{
					...
					{
						strSqlCommand = "SELECT * FROM ABERTURACAIXA  order by  " + FieldOrder;
					}
				}
				else
				{
		...
		private static ABERTURACAIXACollection ExecuteReader(ref ABERTURACAIXACollection collection, ref FbDataReader dataReader, FbCommand dbCommand)
		{
			using (dataReader = dbCommand.ExecuteReader())
			{
				collection = new ABERTURACAIXACollection();
		...
		private static ABERTURACAIXAEntity FillEntityObject(ref FbDataReader DataReader) 
		{
			ABERTURACAIXAEntity entity = new ABERTURACAIXAEntity();

			FirebirdGetDbData getData = new FirebirdGetDbData();

							entity.IDABERTURACAIXA = getData.ConvertDBValueToInt32(DataReader, DataReader.GetOrdinal("IDABERTURACAIXA"));
